...
test('raw 26.1.2 offline identity keeps original casing and case-sensitive UUID ownership', { timeout: 45_000 }, async () => {
  const port = await reservePort()
  const root = await createTempWorld('rustcraft-identity-normalization-')
  const names = ['NormCase', 'normcase', 'NORMCASE']
  let server
  ...
  try {
    ...
    assert.equal(new Set(joins.map(joined => joined.joinState.profile.uuid)).size, names.length)
    ...
    for (const username of names) {
      ...
      assert.equal(matches.length, 1, `${username} should retain original casing in usercache`)
