...
- [x] Add parity test: `level.dat` written and re-read with same fields for a default world vs. vanilla-generated `level.dat` — `default_level_dat_round_trip_matches_vanilla_generated_field_shape` asserts the default-world vanilla field surface and exact `PrimaryLevelData` readback; covered by `cargo test -q level_dat`.
