...
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/loot/packs/VanillaLootTableProvider.java`. Rust `data_loot_provider` covers Java's vanilla provider factory, required table source from `BuiltInLootTables.all()`, the exact 12 `SubProviderEntry` registrations in order, context param sets for fishing/chest/entity/equipment/block/piglin barter/gift/archaeology/shearing/entity interact/block interact/charged creeper entity, registry future forwarding, and source counts/sentinels for the wrapper construction; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 vanilla_loot_table_provider`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/loot/packs/VanillaPiglinBarterLoot.java`. Rust `data_loot_piglin_barter` covers Java's single `BuiltInLootTables.PIGLIN_BARTERING` table, constant one-roll pool, all 19 ordered weighted entries, soul-speed random enchantments for book and iron boots, fire-resistance/water potion functions, all uniform and constant count functions including `Items.DRIED_GHAST`, and exact source counts for accepts, pools, entries, weights, count/potion/enchantment functions, uniform ranges, and constant values; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 piglin_barter`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/loot/packs/VanillaShearingLoot.java`. Rust `data_loot_shearing` covers Java's bogged shearing mushrooms/counts, 16 dye-specific sheep shearing tables through `LootData.WOOL_ITEM_BY_DYE`, uniform 1..3 wool rolls, sheep dye dispatch table, mooshroom red/brown variant alternatives with component predicates, red/brown mooshroom five-roll mushroom tables, snow golem carved-pumpkin table, and exact source counts for accepts, pools, item/nested entries, alternatives, count functions, entity property predicates, component exact predicates, and number providers; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 shearing`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/loot/packs/package-info.java`. Rust `data_loot_packs` covers the package's `@NullMarked` annotation, package declaration, and jspecify import; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 loot_packs_package_info`.
...
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/package-info.java`. Rust `data_package` covers the package's `@NullMarked` annotation, package declaration, and jspecify import; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 package_is_null_marked`.
...
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/recipes/RecipeOutput.java`. Rust `data_recipes` covers Java's recipe accept contract with nullable advancement holder, advancement builder accessor, root advancement inclusion hook, and source counts/sentinels; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 recipe_output`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/recipes/RecipeProvider.java`. Rust `data_recipes` covers Java's provider helper contracts for recipe naming, one-to-one conversion, ore cooking save ids, structural shaped/shapeless helper patterns, fence and hanging-sign edge cases, nine-block packing/unpacking, smithing-template copy, food cooking set, block-family crafting/stonecutter dispatch maps, criteria/name helper sentinels, runner duplicate detection, advancement parent/root behavior, stable recipe/advancement save hooks, and source counts/sentinels; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 recipe_provider`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/recipes/RecipeUnlockAdvancementBuilder.java`. Rust `data_recipe_unlock_advancement` covers Java's insertion-ordered `LinkedHashMap` criteria storage, duplicate-key replacement without reordering, empty-criteria exception text, recipe-unlocked criterion, recipe reward, OR requirements strategy, output advancement parent contract, category folder path prefix, and source counts/sentinels; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 recipe_unlock_advancement`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/recipes/ShapedRecipeBuilder.java`. Rust `data_shaped_recipe_builder` covers Java's shaped factory overloads, default count and default id, linked key storage, tag/item/ingredient define forwarding, duplicate-symbol and whitespace-symbol validation text, equal-width pattern validation, unlock forwarding, nullable group handling, show-notification flag, save construction of shaped pattern/common info/book info/result/advancement, empty-unlock failure through the advancement builder, and source counts/sentinels; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 shaped_recipe_builder`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/recipes/ShapelessRecipeBuilder.java`. Rust `data_shapeless_recipe_builder` covers Java's shapeless factory overloads, ItemStackTemplate/result count handling, tag/item/ingredient `requires` forwarding, repeated ingredient insertion for counts, unlock forwarding and replacement semantics through the advancement builder, nullable group handling, default id, save construction with notifications always enabled, crafting book info, result/ingredients passthrough, advancement id prefix, empty-unlock failure, and source counts/sentinels; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 shapeless_recipe_builder`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/recipes/SimpleCookingRecipeBuilder.java`. Rust `data_simple_cooking_recipe_builder` covers Java's generic/campfire/blasting/smelting/smoking factory selection, food-category defaults for campfire and smoking, result stack/default id, ingredient/experience/cooking-time retention, unlock forwarding and replacement semantics through the advancement builder, nullable group handling with `Objects.requireNonNullElse`, save construction with notifications always enabled, direct `CookingBookInfo`, factory invocation fields, advancement id prefix, empty-unlock failure, and source counts/sentinels; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 simple_cooking_recipe_builder`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/recipes/SingleItemRecipeBuilder.java`. Rust `data_single_item_recipe_builder` covers Java's stonecutting factory, result stack/count/default id, ingredient and category retention, no-op nullable group method, unlock forwarding and replacement semantics through the advancement builder, save construction with direct `Recipe.CommonInfo(true)`, factory invocation fields, advancement id prefix, empty-unlock failure, and source counts/sentinels; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 single_item_recipe_builder`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/recipes/SmithingTransformRecipeBuilder.java`. Rust `data_smithing_transform_recipe_builder` covers Java's smithing factory, template/base/addition/category/result storage, `unlocks` forwarding and replacement semantics through the advancement builder, string-id save conversion to recipe `ResourceKey`, key save construction with direct `Recipe.CommonInfo(true)`, optional template/addition wrapping, base/result passthrough, advancement id prefix, empty-unlock failure, and source counts/sentinels; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 smithing_transform_recipe_builder`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/recipes/SmithingTrimRecipeBuilder.java`. Rust `data_smithing_trim_recipe_builder` covers Java's smithing-trim factory, category/template/base/addition/trim-pattern holder storage, `unlocks` forwarding and replacement semantics through the advancement builder, key-only save construction with direct `Recipe.CommonInfo(true)`, template/base/addition/pattern passthrough without optionals, advancement id prefix, empty-unlock failure, and source counts/sentinels; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 smithing_trim_recipe_builder`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/recipes/SpecialRecipeBuilder.java`. Rust `data_special_recipe_builder` covers Java's supplier factory storage/invocation, static `special` factory, lazily created nullable advancement builder, optional null advancement when no unlocks are registered, `unlockedBy` forwarding and replacement semantics, string-id save conversion to recipe `ResourceKey`, key save with `RecipeCategory.MISC` advancement path, and source counts/sentinels; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 special_recipe_builder`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/recipes/TransmuteRecipeBuilder.java`. Rust `data_transmute_recipe_builder` covers Java's item and stack transmute factories, category/result/input/material storage, default material-count bounds, `addMaterialCountToOutput` flag, `setMaterialCount`, unlock forwarding and replacement semantics through the advancement builder, nullable group handling, default id, save construction with notifications enabled, crafting book info, input/material/material-count/result/count-flag passthrough, advancement id prefix, empty-unlock failure, and source counts/sentinels; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 transmute_recipe_builder`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/recipes/package-info.java`. Rust `data_recipes_package` covers the package's `@NullMarked` annotation, package declaration, and jspecify import; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 recipes_package`.
...
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/tags/BannerPatternTagsProvider.java`. Rust `data_tags_registry_providers` covers the banner-pattern registry provider, `NO_ITEM_REQUIRED` pattern set boundaries, all pattern-item tag sentinels from flower through bordure-indented, provider call counts, registry constructor, and source line count; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 data_tags_registry_providers`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/tags/BiomeTagsProvider.java`. Rust `data_tags_biomes` covers the biome registry tag provider, key-provider constructor, source line count, tag/add/addTag/addAll/biome-reference counts, repeated parent tag reference counts, overworld/nether `addAll` flows, trial-chambers deep-dark filter, structure biome tags, climate variant spawn tags, and current biome sentinels including `CHERRY_GROVE`, `PALE_GARDEN`, `DEEP_LUKEWARM_OCEAN`, and `SPAWNS_CORAL_VARIANT_ZOMBIE_NAUTILUS`; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 data_tags_biomes`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/tags/BlockItemTagsProvider.java`. Rust `data_tags_block_items` covers the abstract paired block/item tag provider, full source line count, all paired tag builder/add/addTag/addAll/block-tag/item-tag/block-reference counts, two flower array definitions and reuse, abstract `TagAppender<Block, Block>` contract, parent tag reference shape, and current block sentinels including pale oak, copper doors/chests/golem statues, copper chains/lanterns/bars, resin bricks, wildflowers, cactus flower, eyeblossoms, and golden dandelion; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 data_tags_block_items`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/tags/DamageTypeTagsProvider.java`. Rust `data_tags_medium_providers` covers the damage-type registry provider, all 33 tag call counts, add/addTag counts, armor/bypass/fire/projectile/explosion/player-attack/panic/wolf-armor/mace-smash sentinel coverage, and source line count; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 data_tags_medium_providers`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/tags/DialogTagsProvider.java`. Rust `data_tags_registry_providers` covers the dialog registry provider and its two intentionally empty tag builders for pause-screen additions and quick actions; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 data_tags_registry_providers`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/tags/EnchantmentTagsProvider.java`. Rust `data_tags_medium_providers` covers the abstract enchantment tag provider, enchantment registry constructor, tooltip-order tag append, complete-registry check against `Registries.ENCHANTMENT`, missing-enchantment error sentinel, and source line count; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 data_tags_medium_providers`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/tags/EntityTypeTagsProvider.java`. Rust `data_tags_entity_types` covers the intrinsic entity-type registry provider, built-in holder key extractor, all tag/add/addTag/entity-reference call counts, repeated parent tag reference counts, source line count, and representative modern entity sentinels including `PARCHED`, `ZOMBIE_NAUTILUS`, `COPPER_GOLEM`, `NAUTILUS`, `HAPPY_GHAST`, `BAMBOO_RAFT`, `CAMEL_HUSK`, and `CREAKING`; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 data_tags_entity_types`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/tags/FeatureTagsProvider.java`. Rust `data_tags_registry_providers` covers the configured-feature registry provider and the `CAN_SPAWN_FROM_BONE_MEAL` vegetation feature set sentinels including default, cherry, wildflower, and pale-garden entries; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 data_tags_registry_providers`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/tags/FlatLevelGeneratorPresetTagsProvider.java`. Rust `data_tags_registry_providers` covers the flat-level-generator-preset registry provider and all visible preset chain sentinels from classic flat through the void; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 data_tags_registry_providers`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/tags/FluidTagsProvider.java`. Rust `data_tags_registry_providers` covers the intrinsic fluid provider, built-in-holder key extractor, water/lava source and flowing entries, support tags, water tag reference, call counts, and registry constructor; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 data_tags_registry_providers`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/tags/GameEventTagsProvider.java`. Rust `data_tags_medium_providers` covers the game-event registry provider, visible-for-testing `VIBRATIONS_EXCEPT_FLAP` list boundaries, vibration/warden/shrieker/sneaking/allay tag composition, resonance-event `addAll` usage, call counts, and source line count; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 data_tags_medium_providers`.
...
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/tags/InstrumentTagsProvider.java`. Rust `data_tags_registry_providers` covers the instrument registry provider, regular and screaming goat horn entry boundaries, and `GOAT_HORNS` tag composition from both child tags; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 data_tags_registry_providers`.
...
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/tags/PaintingVariantTagsProvider.java`. Rust `data_tags_registry_providers` covers the painting-variant registry provider and placeable painting sentinels spanning legacy and modern variants from `KEBAB` through `DENNIS`; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 data_tags_registry_providers`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/tags/PoiTypeTagsProvider.java`. Rust `data_tags_registry_providers` covers the POI registry provider, acquirable job-site bounds, village tag composition from job sites/home/meeting, and bee-home entries; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 data_tags_registry_providers`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/tags/PotionTagsProvider.java`. Rust `data_tags_medium_providers` covers the holder-based potion registry provider, `TRADEABLE` potion set boundaries from wind-charged through long slow falling, exact potion reference count, and source line count; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 data_tags_medium_providers`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/tags/StructureTagsProvider.java`. Rust `data_tags_medium_providers` covers the structure registry provider, village/mineshaft/ocean-ruin/shipwreck/ruined-portal/map-target tags, dolphin located tag composition, exact built-in structure reference count, call counts, and source line count; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 data_tags_medium_providers`.
...
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/tags/TimelineTagsProvider.java`. Rust `data_tags_registry_providers` covers the timeline registry provider, universal schedule entry, overworld timeline composition, and nether/end inheritance from universal; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 data_tags_registry_providers`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/tags/TradeRebalanceEnchantmentTagsProvider.java`. Rust `data_tags_medium_providers` covers all seven biome-specific trade-rebalance enchantment tags and their three-enchantment sentinels from desert through taiga; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 data_tags_medium_providers`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/tags/TradeRebalanceTradeTagsProvider.java`. Rust `data_tags_trade_rebalance_trades` covers the villager-trade registry provider, all ten replace=true librarian/armorer level tags, vanilla-vs-rebalance entry totals, level-shape counts, representative librarian and armorer trade sentinels from desert/jungle/plains/savanna/snow/swamp/taiga groups, and source call/line counts; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 data_tags_trade_rebalance_trades`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/tags/VanillaBlockTagsProvider.java`. Rust `data_tags_vanilla_blocks` covers the intrinsic block registry provider, built-in holder key extractor, embedded `BlockItemTagsProvider` bridge, full source line count, tag/add/addTag/addAll/block-tag/block-reference counts, repeated parent tag reference counts, dynamic registry-derived `REPLACEABLE` tag, empty incorrect-tool/frogspawn-support tags, tool-tier and mineable tags, spawn/support/environment tags, ambient sound trigger tags, and current sentinels including pale oak, eyeblossoms, golden dandelion, cactus flower, test blocks, copper families, resin blocks, creaking heart, leaf litter, happy ghast avoids, dried ghast ambient sounds, and `AIR`; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 data_tags_vanilla_blocks`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/tags/VanillaEnchantmentTagsProvider.java`. Rust `data_tags_vanilla_enchantments` covers the vanilla enchantment tag provider, tooltip-order sequence for all 43 enchantments, exclusive/treasure/non-treasure/loot/trade/prevention tag sentinels, add/addTag/tag call counts, enchantment reference count, and source line count; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 data_tags_vanilla_enchantments`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/tags/VanillaItemTagsProvider.java`. Rust `data_tags_vanilla_items` covers the intrinsic item registry provider, built-in holder key extractor, embedded `BlockItemTagsProvider` bridge, `BlockToItemConverter` block-to-item mapping and optional forwarding, full source line count, tag/add/addTag/optional/item-tag/item-reference counts, repeated parent tag reference counts, equipment/enchantment/food/material tag groups, and current sentinels including pale oak boats, bundles, blue/brown eggs, golden nautilus armor, golden dandelion, copper tools/armor/materials, spears and lunge enchantability, resin brick trim materials, harnesses, happy ghast tempt items, camel husk food, nautilus food, copper golem shearables, copper nuggets, and loom patterns; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 data_tags_vanilla_items`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/tags/VillagerTradesTagsProvider.java`. Rust `data_tags_villager_trades` covers the villager-trade registry key provider, full source line count, tag/add/addTag/villager-trade-key/reference counts, profession/level tag shape, empty common-smith level 3-5 tags, smith common-level inheritance, cartographer exploration map sentinels, mason typo-preserving `POLISHED_GRANTITE` sentinel, wandering trader buying/uncommon/common groups, and current pale-oak/open-eyeblossom/wildflowers/firefly-bush/golden-dandelion trade sentinels; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 data_tags_villager_trades`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/tags/WorldPresetTagsProvider.java`. Rust `data_tags_registry_providers` covers the world-preset registry provider, normal preset set sentinels, and extended tag composition from normal plus debug; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 data_tags_registry_providers`.
...
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/worldgen/AncientCityStructurePieces.java`. Rust `data_worldgen_jigsaw_pools` covers the ancient-city start pool key, processor lookup, city-center entries, chained `AncientCityStructurePools.bootstrap`, source line count, registration/element/bootstrap counts, and `JIGSAW_STRUCTURE_START_POOLS` metadata; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 data_worldgen_jigsaw_pools`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/worldgen/AncientCityStructurePools.java`. Rust `data_worldgen_jigsaw_pools` covers ancient-city structures/sculk/walls/city entrance pool registration counts, sculk feature lookup, generic/walls degradation processors, list and empty pool elements, source line count, and Rust jigsaw bootstrap metadata; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 data_worldgen_jigsaw_pools`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/worldgen/BastionBridgePools.java`. Rust `data_worldgen_jigsaw_pools` covers bridge starting/bridge/legs/walls/ramparts/plates/connectors pool registration counts, entrance/bridge/rampart processor sentinels, source line count, element counts, and Rust jigsaw bootstrap metadata; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 data_worldgen_jigsaw_pools`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/worldgen/BastionHoglinStablePools.java`. Rust `data_worldgen_jigsaw_pools` covers hoglin stable starting/large/small/post/rampart/stair pool registration counts, stable/rampart processor sentinels, source line count, element counts, and Rust jigsaw bootstrap metadata; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 data_worldgen_jigsaw_pools`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/worldgen/BastionHousingUnitsPools.java`. Rust `data_worldgen_jigsaw_pools` covers housing unit center/path/stage/wall pool registration counts, housing processor sentinels, source line count, element counts, and Rust jigsaw bootstrap metadata; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 data_worldgen_jigsaw_pools`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/worldgen/BastionPieces.java`. Rust `data_worldgen_jigsaw_pools` covers the bastion start pool key, four weighted start pieces, chained housing/hoglin/treasure/bridge/shared bootstraps, source line count, registration/element/bootstrap counts, and `JIGSAW_STRUCTURE_START_POOLS` metadata; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 data_worldgen_jigsaw_pools`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/worldgen/BastionSharedPools.java`. Rust `data_worldgen_jigsaw_pools` covers shared bastion gold/block/mob pool registration counts, empty-template-pool lookup, no-processor single element sentinels, source line count, element counts, and Rust jigsaw bootstrap metadata; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 data_worldgen_jigsaw_pools`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/worldgen/BastionTreasureRoomPools.java`. Rust `data_worldgen_jigsaw_pools` covers treasure-room base/center/wall/extension/corner/stair pool registration counts, treasure/high-rampart/generic processor sentinels, source line count, element counts, and Rust jigsaw bootstrap metadata; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 data_worldgen_jigsaw_pools`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/worldgen/BiomeDefaultFeatures.java`. Rust `data_worldgen_biome_defaults` covers Java source line/method/addFeature/addCarver/addSpawn/category/placement counts, default carvers/lakes/monster rooms/ores/cave vegetation/spawn sentinels, representative Rust plains/lush-caves/dripstone/deep-dark feature-step outputs, and representative farm/desert/swamp/dripstone spawn weights including zombie horse, parched, bogged, and drowned; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 data_worldgen_biome_defaults`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/worldgen/BootstrapContext.java`. Rust `data_worldgen_bootstrap_context` covers the Java interface imports/signatures, explicit-lifecycle register method, default stable-lifecycle register overload, generic lookup method, and the Rust registry mapping for stable/default registration, explicit lifecycle preservation, returned resource-key references, lookup boundaries, and duplicate-key rejection; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 data_worldgen_bootstrap_context`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/worldgen/Carvers.java`. Rust `data_worldgen_carvers` covers the Java configured-carver resource keys, configured-carver registry lookup, four bootstrap registrations, cave/extra-cave/canyon/nether-cave constructor parameters, debug barrier states, replaceable block tags, canyon shape settings, Rust `CONFIGURED_CARVERS` table parity, namespaced/unqualified lookup, and bundled vanilla configured-carver JSON decoding; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 data_worldgen_carvers`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/worldgen/DesertVillagePools.java`. Rust `data_worldgen_jigsaw_pools` covers desert village start/terminator/zombie terminator keys, 12 bootstrap registrations, placed-feature and processor lookups, Java source sentinels for town centers, streets, houses, zombie houses, decor, camel, and villagers, Rust jigsaw bootstrap/start-pool metadata, and vanilla desert template-pool JSON parity for pool IDs, fallbacks, weights, processors, features, and representative entries; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 data_worldgen_jigsaw_pools`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/worldgen/DimensionTypes.java`. Rust `data_worldgen_dimension_types` covers Java bootstrap lookup/register shape, the four dimension type registrations, overworld/nether/end/overworld-caves constructor sentinels, monster spawn settings, skybox/cardinal-light choices, timeline/default-clock lookups, vanilla dimension_type JSON field parity, and Rust builtin dimension-type registry id/order parity; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 data_worldgen_dimension_types`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/worldgen/NoiseData.java`. Rust `data_worldgen_noise_data` covers the Java bootstrap shape, `DEFAULT_SHIFT`, biome-noise helper calls, 62 `Noises.*` registrations, representative cave/jagged/nether-layer sentinels, Rust `NORMAL_NOISE_PARAMETERS` registration order and values, lookup aliases, and full vanilla `worldgen/noise` JSON parity for every registered noise parameter; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 data_worldgen_noise_data`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/worldgen/PillagerOutpostPools.java`. Rust `data_worldgen_jigsaw_pools` covers the outpost start key, four bootstrap registrations, `OUTPOST_ROT` processor lookup, empty fallback lookup, Java source sentinels for base plate, tower list, feature plate, features, allay cage, and empty feature weight, Rust jigsaw bootstrap/start-pool metadata, and vanilla pillager-outpost template-pool JSON parity for pool IDs, fallbacks, weights, projections, list children, processors, and feature entries; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 data_worldgen_jigsaw_pools`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/worldgen/PlainVillagePools.java`. Rust `data_worldgen_jigsaw_pools` covers the plains start/terminator keys, 17 bootstrap registrations including shared `village/common` animal pools, placed-feature and processor lookups, Java source sentinels for town centers, streets, houses, farms, decor, villagers, cats, and common utility pools, Rust jigsaw bootstrap/start-pool metadata, and vanilla plains/common template-pool JSON parity for pool IDs, fallbacks, weights, processors, projections, features, and representative entries; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 data_worldgen_jigsaw_pools`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/worldgen/Pools.java`. Rust `data_worldgen_jigsaw_pools` covers the `EMPTY` pool key, `createKey`/`parseKey`/`register` helper shape, two root registrations, empty-pool fallback and empty element list, and chained bootstraps for bastions, pillager outposts, villages, ancient cities, trail ruins, and trial chambers through Rust jigsaw bootstrap metadata and vanilla `template_pool/empty.json`; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 data_worldgen_jigsaw_pools`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/worldgen/ProcessorLists.java`. Rust `data_worldgen_processor_lists` covers the 40 processor-list keys, 40 bootstrap registrations, helper/register shape, block lookup, bastion gilded-blackstone rules, block-rot/protected/capped processor sentinels, fossil/ancient-city/trail-ruins/trial-chambers special cases, Rust `STRUCTURE_PROCESSOR_LISTS` key parity, and full vanilla `worldgen/processor_list` JSON coverage for processor-list IDs, processor counts, processor types, predicate types, output states, capped archaeology delegates, protected tags, and rot integrity values; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 data_worldgen_processor_lists`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/worldgen/SavannaVillagePools.java`. Rust `data_worldgen_jigsaw_pools` covers the savanna start/terminator/zombie terminator keys, 12 bootstrap registrations, placed-feature and processor lookups, Java source sentinels for town centers, streets, houses, zombie houses, terminators, trees, decor, and villagers, Rust jigsaw bootstrap/start-pool metadata, and vanilla savanna template-pool JSON parity for pool IDs, fallbacks, weights, processors, projections, feature entries, and empty entries; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 data_worldgen_jigsaw_pools`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/worldgen/SnowyVillagePools.java`. Rust `data_worldgen_jigsaw_pools` covers the snowy start/terminator keys, 11 bootstrap registrations, placed-feature and processor lookups, Java source sentinels for town centers, streets, houses, zombie houses, terminators, trees, decor, and villagers, Rust jigsaw bootstrap/start-pool metadata, and vanilla snowy template-pool JSON parity for pool IDs, fallbacks, weights, processors, projections, feature entries, and empty entries; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 data_worldgen_jigsaw_pools`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/worldgen/StructureSets.java`. Rust `data_worldgen_structure_sets` covers the 20 Java structure-set registrations, structure/biome lookups, 19 random-spread placements, the stronghold concentric-rings placement, village/outpost/buried-treasure/mineshaft/nether-complex sentinels, Rust `BUILTIN_STRUCTURE_SETS` order and placement parity, and vanilla `worldgen/structure_set` JSON parity for IDs, structure entries, weights, spacing/separation/salt/spread type, frequency reduction, locate offset, exclusion zones, and stronghold preferred biomes; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 data_worldgen_structure_sets`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/worldgen/Structures.java`. Rust `data_worldgen_structures` covers the 34 Java structure registrations, biome/template lookups, jigsaw/ruined-portal/mineshaft/ocean-ruin/shipwreck constructor counts, spawn override, terrain adaptation, generation-step, ancient-city, trial-chambers, village, and ruined-portal sentinels, Rust `BUILTIN_STRUCTURES` order parity, and vanilla `worldgen/structure` JSON parity for IDs, type distribution, generation steps, terrain adaptation distribution, jigsaw start pools and sizes, spawn overrides, ruined portal setups, ancient-city height/distance, and trial-chambers aliases/liquid settings/dimension padding; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 data_worldgen_structures`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/worldgen/SurfaceRuleData.java`. Rust `data_worldgen_surface_rule_data` covers the Java surface-rule preset source shape, block state-rule constants, overworld/overworldLike/nether/end/air method sentinels, surface-rule constructor counts, Rust `BUILTIN_SURFACE_RULE_PRESETS` method parity, representative overworld/nether block coverage, and vanilla `noise_settings/*` `surface_rule` root parsing through the Rust dynamic surface-rule codec; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 data_worldgen_surface_rule_data`. Broader `SurfaceRules` runtime semantics remain tracked separately in `CHECKLIST_WORLDGEN.md`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/worldgen/TaigaVillagePools.java`. Rust `data_worldgen_jigsaw_pools` covers the taiga start/terminator keys, 10 bootstrap registrations, placed-feature and processor lookups, Java source sentinels for town centers, streets, houses, zombie houses, terminators, decor, and villagers, Rust jigsaw bootstrap/start-pool metadata, and vanilla taiga template-pool JSON parity for pool IDs, fallbacks, weights, processors, projections, feature entries, and empty entries; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 data_worldgen_jigsaw_pools`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/worldgen/TerrainProvider.java`. Rust `worldgen::terrain_splines` covers Java-shaped `CubicSpline` Hermite interpolation, finite min/max bound propagation, TerrainProvider overworld offset/factor/jaggedness builders, normal/large-biomes/amplified variants, TerrainProvider/CubicSpline source-count and sentinel audits, and sample regressions including ocean, far-inland, and nonzero jaggedness contexts; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 terrain_`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/worldgen/TrailRuinsStructurePools.java`. Rust `data_worldgen_jigsaw_pools` covers the trail-ruins start key, seven bootstrap registrations, houses/roads/tower-top archaeology processor lookups, Java source sentinels for tower, tower-top, additions, roads, buildings, grouped buildings, and decor entries, Rust jigsaw bootstrap/start-pool metadata, and vanilla trail-ruins template-pool JSON parity for pool IDs, fallbacks, weights, processors, projections, and representative entries; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 data_worldgen_jigsaw_pools`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/worldgen/TrialChambersStructurePools.java`. Rust `data_worldgen_jigsaw_pools` covers the trial-chambers start and hallway-fallback keys, 34 explicit Java bootstrap registrations, alias-binding source shape for ranged/melee/small-melee spawner targets, copper-bulb degradation processor lookup, Java source sentinels for chamber/corridor/hallway/decor/reward/spawner pools, Rust jigsaw bootstrap/start-pool metadata, and all 47 extracted vanilla trial-chambers template-pool JSON IDs, fallbacks, element counts, weight sums, projections, empty elements, and representative corridor/hallway/decor/spawner entries; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 data_worldgen_jigsaw_pools`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/worldgen/VillagePools.java`. Rust `data_worldgen_jigsaw_pools` covers the 13-line village dispatcher source, zero direct template-pool registrations, chained bootstraps for plains, snowy, savanna, desert, and taiga village pools, Rust zero-registration bootstrap metadata for the dispatcher, and the already-audited per-village parsed template-pool JSON parity; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 data_worldgen_jigsaw_pools` and `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 jigsaw_and_processor_registries_match_vanilla_bootstrap_surface`.
...
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/worldgen/biome/EndBiomes.java`. Rust `biome`/`worldgen` covers the End biome source shape, shared base-End precipitation/temperature/downfall/water-color and `endSpawns` sentinels, all five End biome factory methods, exact End placement feature sentinels, parsed vanilla JSON parity for `the_end`, `end_highlands`, `end_midlands`, `small_end_islands`, and `end_barrens` feature steps and Enderman monster spawn entries against Rust builtin generation models; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 end_biomes`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/worldgen/biome/NetherBiomes.java`. Rust `biome`/`worldgen` covers the Nether biome source shape, shared base Nether precipitation/temperature/downfall/water-color sentinels, all five Nether biome factory methods, nether cave carver usage, feature/spawn/mob-charge counts, default mushroom/ore/ancient-debris helper calls, representative placement and entity spawn sentinels, and parsed vanilla JSON parity for `nether_wastes`, `crimson_forest`, `warped_forest`, `soul_sand_valley`, and `basalt_deltas` carvers, feature steps, creature/monster spawns, and spawn costs against Rust builtin generation models; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 nether_biomes`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/worldgen/biome/OverworldBiomes.java`. Rust `biome`/`worldgen` covers the Overworld biome source shape, 32 public factory methods, private base/helper method counts, global Overworld generation call count, direct carver/feature/spawn counts, default ore/soft-disk/mushroom helper counts, sky-color/base-biome/global-generation/carver/helper/entity/placement sentinels, and parsed vanilla JSON parity for all 55 Overworld biome generation models' carvers, feature steps, creature spawn probabilities, spawns, and spawn costs. The audit fixed `ice_spikes` to use the Java `snowySpawns(mobs, false)` monster table without zombie horses and fixed `bamboo_jungle` to use the Java `ocelot` `1..1` monster bounds; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 overworld_biomes`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/worldgen/biome/package-info.java`. Rust `biome` records that the Java package is metadata-only `@NullMarked` with package declaration `net.minecraft.data.worldgen.biome` and no runtime behavior to port; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 biome_package_info`.
...
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/worldgen/features/CaveFeatures.java`. Rust `worldgen` covers the 24 cave configured-feature bootstrap keys/source entries, Java source line/register/createKey/inline-placement/configuration counts and sentinels, and parsed vanilla JSON parity for every cave configured feature's type plus representative fossil, dripstone-cluster, underwater-magma, moss-patch, lush-caves selector, and sculk-patch configuration fields; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 cave_features`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/worldgen/features/EndFeatures.java`. Rust `worldgen` covers the six End configured-feature bootstrap keys/source entries, Java source line/register/createKey counts and sentinels, and parsed vanilla JSON parity for End platform, spike, return/delayed gateways, chorus plant, and End island configs; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 end_features`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/worldgen/features/FeatureUtils.java`. Rust `worldgen` covers the configured-feature bootstrap dispatcher order, `createKey` default-namespace/`Registries.CONFIGURED_FEATURE` semantics via configured-feature lookup, no-config registration forwarding to `FeatureConfiguration.NONE`, configured registration output shape, and the 221-entry configured-feature registry produced by the nine feature bootstrap sources; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 feature_utils`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/worldgen/features/MiscOverworldFeatures.java`. Rust `worldgen` covers the 18 misc Overworld configured-feature bootstrap keys/source entries, Java source line/register/createKey/disk/spring/provider counts and sentinels, parsed vanilla JSON parity for every misc Overworld configured feature's type, and representative ice-spike, disk sand/grass, and spring configuration fields; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 misc_overworld_features`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/worldgen/features/NetherFeatures.java`. Rust `worldgen` covers the 22 Nether configured-feature bootstrap keys/source entries, Java source line/register/createKey/vegetation/spring/simple-block/vines counts and sentinels, parsed vanilla JSON parity for every Nether configured feature's type, and representative delta, basalt column/blob, Nether forest vegetation, sprouts, twisting vines, and Nether spring configuration fields; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 nether_features`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/worldgen/features/OreFeatures.java`. Rust `worldgen` covers the 32 ore configured-feature bootstrap keys/source entries, Java source line/register/createKey/OreConfiguration/target/scattered-ore counts and sentinels, parsed vanilla JSON parity for every ore configured feature's type, and representative Nether ore, buried ore, target-list, diamond, ancient-debris scattered-ore, and copper configuration fields; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 ore_features`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/worldgen/features/PileFeatures.java`. Rust `worldgen` covers the five pile configured-feature bootstrap keys/source entries, Java source line/register/createKey/BlockPileConfiguration/provider counts and sentinels, parsed vanilla JSON parity for every pile configured feature's `block_pile` type, and rotated/simple/weighted state-provider configuration fields for hay, melon, snow, ice, and pumpkin piles; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 pile_features`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/worldgen/features/TreeFeatures.java`. Rust `worldgen` covers the 50 tree configured-feature bootstrap keys/source entries, Java source line/register/createKey/tree-builder/fallen-tree/fungus/mushroom/decorator/root-placer counts and sentinels, parsed vanilla JSON parity for every tree configured feature's type, and representative huge fungus, mushroom, oak, pale oak, creaking heart, cherry, mangrove, leaf-litter, and fallen-tree configuration fields; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 tree_features`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/worldgen/features/VegetationFeatures.java`. Rust `worldgen` covers the 57 vegetation configured-feature bootstrap keys/source entries, Java source line/register/createKey/simple-block/random-selector/weighted-feature/block-column/vegetation-patch/inline-placement counts and sentinels, parsed vanilla JSON parity for every vegetation configured feature's type, and representative bamboo, simple-block, weighted provider, block-column, flower noise/provider, simple-random selector, pale moss patch, random selector, tree selector, bamboo vegetation, mushroom island, and mangrove vegetation configuration fields; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 vegetation_features`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/worldgen/features/package-info.java`. Rust `worldgen` records that this Java package-info is metadata-only, applies exactly one `@NullMarked` annotation to `net.minecraft.data.worldgen.features`, and imports `org.jspecify.annotations.NullMarked`; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 worldgen_package_info`.

## `decompiled-server-26.1.2/net/minecraft/data/worldgen`

- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/worldgen/package-info.java`. Rust `worldgen` records that this Java package-info is metadata-only, applies exactly one `@NullMarked` annotation to `net.minecraft.data.worldgen`, and imports `org.jspecify.annotations.NullMarked`; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 worldgen_package_info`.
...
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/worldgen/placement/CavePlacements.java`. Rust `worldgen` covers the 20 cave placed-feature bootstrap keys/source entries, Java source line/register/createKey/count/rarity/height-range/environment-scan/random-offset/surface-threshold counts and sentinels, parsed vanilla JSON parity for every cave placed feature's configured-feature reference, and representative monster-room, fossil, dripstone, underwater magma, glow lichen, lush-cave scan, vines, amethyst geode, sculk patch, and sculk vein placement modifiers; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 cave_placements`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/worldgen/placement/EndPlacements.java`. Rust `worldgen` covers the five End placed-feature bootstrap keys/source entries, Java source line/register/createKey/rarity/heightmap/biome counts and sentinels, parsed vanilla JSON parity for every End placed feature's configured-feature reference, and representative fixed End platform, biome-only spike, gateway rarity/heightmap/vertical-offset, chorus count/heightmap, and decorated island rarity/count-extra/height-range placement modifiers; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 end_placements`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/worldgen/placement/MiscOverworldPlacements.java`. Rust `worldgen` covers the 18 misc-overworld placed-feature bootstrap keys/source entries, Java source line/register/createKey/count/rarity/height-range/heightmap/block-predicate/environment-scan/surface-threshold/random-offset counts and sentinels, parsed vanilla JSON parity for every misc-overworld placed feature's configured-feature reference, and representative ice, iceberg, blue ice, lava lake, disk, biome-only, desert well, lava spring, frozen lava spring, and water spring placement modifiers; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 misc_overworld_placements`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/worldgen/placement/NetherPlacements.java`. Rust `worldgen` covers the 20 Nether placed-feature bootstrap keys/source entries, Java source line/register/createKey/count-on-layer/count/full-range/range/block-predicate/random-offset/fire-placement counts and sentinels, parsed vanilla JSON parity for every Nether placed feature's configured-feature reference, and representative count-on-every-layer, full-range, glowstone, spring, crimson-roots, soul-fire, and fire patch placement modifiers; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 nether_placements`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/worldgen/placement/OrePlacements.java`. Rust `worldgen` covers the 40 ore placed-feature bootstrap keys/source entries, Java source line/register/createKey/common-ore/rare-ore/ore-placement/uniform-height/triangle-height/range-constant counts and sentinels, parsed vanilla JSON parity for every ore placed feature's configured-feature reference, and representative Nether ore, stone ore, rarity, overworld trapezoid, uniform count, ancient-debris, copper, and clay placement modifiers; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 ore_placements`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/worldgen/placement/PlacementUtils.java`. Rust `worldgen` now models the shared PlacementUtils bootstrap order, placed-feature key namespacing, heightmap constants, full/range height constants, `countExtra` weight validation/distribution, empty/survival block filters, and inline/filtered placed-feature wrappers; source tests pin Java line/static-constant/bootstrap/register/inline/filter counts and sentinels, and registry tests verify the nine Java bootstrap classes match Rust placed-feature source ordering; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 placement_utils`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/worldgen/placement/TreePlacements.java`. Rust `worldgen` covers the 41 tree placed-feature bootstrap keys/source entries, Java source line/register/createKey/survival-filter/count-on-layer/biome/environment-scan/block-predicate counts and sentinels, parsed vanilla JSON parity for every tree placed feature's configured-feature reference, and representative fungi count-on-layer+biome, checked-tree sapling survival, leaf-litter/fallen-tree survival, and snow-tree environment-scan/block-predicate placement modifiers; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 tree_placements`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/worldgen/placement/VegetationPlacements.java`. Rust `worldgen` covers the 89 vegetation placed-feature bootstrap keys/source entries, Java source line/register/createKey/count/rarity/count-extra/noise-count/surface-water-depth counts and sentinels, parsed vanilla JSON parity for every vegetation placed feature's configured-feature reference, and representative bamboo, vine height-range, patch random-offset/block-predicate, mushroom, flower/noise-threshold, tree count-extra/survival, surface-water-depth, mangrove, sugar-cane, and firefly-bush near-water placement modifiers; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 vegetation_placements`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/worldgen/placement/VillagePlacements.java`. Rust `worldgen` covers the 13 village placed-feature bootstrap keys/source entries, Java source line/register/createKey/survival-filter counts and sentinels, parsed vanilla JSON parity for every village placed feature's configured-feature reference, and representative pile no-modifier, village tree sapling-survival, cactus/flower/taiga-grass/berry count, random-offset, air/survival, and grass-block predicate placement modifiers; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 village_placements`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/worldgen/placement/package-info.java`. Rust `worldgen` records that this Java package-info is metadata-only, applies exactly one `@NullMarked` annotation to `net.minecraft.data.worldgen.placement`, and imports `org.jspecify.annotations.NullMarked`; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 worldgen_placement_package_info`.
...
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/gametest/framework/BlockBasedTestInstance.java`. Rust `gametest_resources` covers the block-based test-instance codec shape (`TestData.CODEC.forGetter(GameTestInstance::info)`), `minecraft:block_based` resource decoding, one-start-block validation, missing-accept failure, accept-trigger success, fail-trigger message failure, and log-trigger reset counting; full `GameTestHelper` structure scanning and block-entity mutation are explicitly deferred by `TODO(gametest-block-based-world-scan)` until the GameTest framework is wired. Verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 block_based_test_instance`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/gametest/framework/BuiltinTestFunctions.java`. Rust `gametest_resources` covers the built-in test-function bootstrap contract: `minecraft:always_pass` is the sole built-in function key, maps to `GameTestHelper::succeed`, is registered by `load`, and is returned from `bootstrap` after loader registration/run; vanilla `always_pass.json` decoding is tied to the same built-in id. Verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 builtin_test_functions`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/gametest/framework/ExhaustedAttemptsException.java`. Rust `gametest_resources` covers the package-private `Throwable` wrapper's exact retry-exhaustion message format, use of attempts/successes/requiredSuccesses/maxAttempts values, and preservation of `testInfo.getError()` as the cause; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 exhausted_attempts_exception`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/gametest/framework/FailedTestTracker.java`. Rust `gametest_resources` covers the static `LAST_FAILED_TESTS` set contract: last-failed test holder ids are remembered uniquely, exposed as a stream-equivalent snapshot, and cleared by `forgetFailedTests`; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 failed_test_tracker`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/gametest/framework/FunctionGameTestInstance.java`. Rust `gametest_resources` covers the function test-instance codec fields (`function` TEST_FUNCTION key plus `TestData`), vanilla function instance decoding, registry lookup run behavior, exact missing-function error message, consumer invocation outcome, type translation key, and function description row; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 function_gametest_instance`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/gametest/framework/GameTestAssertException.java`. Rust `gametest_resources` covers the assertion exception's stored message component string, tick value, superclass runtime-message behavior, `test.error.tick` description key/arguments, and `getMessage()` delegating to the description string; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 gametest_assert_exception`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/gametest/framework/GameTestAssertPosException.java`. Rust `gametest_resources` covers the positional assertion exception's base assertion inheritance, stored absolute/relative block positions, `test.error.position` description key and argument order, original message returned for block display, and absolute/relative position accessors; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 gametest_assert_pos_exception`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/gametest/framework/GameTestBatch.java`. Rust `gametest_resources` covers the Java record fields (`index`, `gameTestInfos`, `environment`) and the constructor invariant that empty test collections throw `IllegalArgumentException` with the exact vanilla message; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 gametest_batch`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/gametest/framework/GameTestBatchFactory.java`. Rust `gametest_resources` covers the Java factory's `MAX_TESTS_PER_BATCH` value, direct decorator defaults (`Rotation.NONE` plus `RetryOptions.noRetries()`), grouping by test environment, 50-item and caller-provided partitioning, null filtering in `fromGameTestInfo`, and `toGameTestBatch` delegation through the batch constructor; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 gametest_batch_factory`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/gametest/framework/GameTestBatchListener.java`. Rust `gametest_resources` covers the listener interface's `testBatchStarting(GameTestBatch)` and `testBatchFinished(GameTestBatch)` callbacks with a recording listener model preserving callback order and batch payloads; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 gametest_batch_listener`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/gametest/framework/GameTestEnvironments.java`. Rust `gametest_resources` covers the `DEFAULT` name, `DEFAULT_KEY` as the TEST_ENVIRONMENT registry key `minecraft:default`, and bootstrap registration of `TestEnvironmentDefinition.AllOf(List.of())`; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 gametest_environments`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/gametest/framework/GameTestEvent.java`. Rust `gametest_resources` covers the event's nullable `expectedDelay` and `minimumDelay` fields, assertion payload, and all three factory methods (`create`, `create(expectedTick, ...)`, and `createWithMinimumDelay`) setting only the Java-matching delay field; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 gametest_event`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/gametest/framework/GameTestException.java`. Rust `gametest_resources` covers the abstract base extending runtime-exception behavior by preserving the constructor message as the runtime message and requiring concrete subclasses to provide a description component equivalent; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 gametest_exception`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/gametest/framework/GameTestHelper.java`. Rust `gametest_resources` pins the full 1150-line helper source surface and 165 method/member signatures, covers pure helper behavior for assertion construction at the current test tick, positional assertion absolute/relative coordinates, single-final-clause enforcement with the exact vanilla error message, run-before/end and run-after-delay scheduling ticks, rotation-dependent absolute/relative block positions, test direction and absolute direction rotation, and relative bounds X/Z swapping for quarter rotations. Live block/entity/world mutation APIs are explicitly deferred behind `gametest-helper-world-runtime` until the server-backed GameTest runtime exists; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 gametest_helper`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/gametest/framework/GameTestInfo.java`. Rust `gametest_resources` pins the 361-line lifecycle source and 60 method/member signatures, covers start-execution negative tick offset, scheduled run-at-tick removal, timeout failure keys for no-result paths, succeed/fail state predicates, required/optional/flaky/retry accessors, rotation composition with extra rotation, sequence counting, and `copyReset` preserving identity/position/retry options while clearing runtime state. Live structure placement, server-level ticking, block-entity lookup, entity cleanup, and listener dispatch are explicitly deferred behind `gametest-info-server-runtime`; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 gametest_info`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/gametest/framework/GameTestInstance.java`. Rust `gametest_resources` covers the abstract base's TEST_INSTANCE_TYPE dispatch registration order (`block_based`, then `function`), TestData-backed accessors for batch/structure/ticks/required/manual/retry/sky-access/rotation/padding, type and info description row order, and description row translation keys/style payloads; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 gametest_instance`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/gametest/framework/GameTestInstances.java`. Rust `gametest_resources` covers the `minecraft:always_pass` TEST_INSTANCE key, bootstrap lookup of test functions and environments, registration of a `FunctionGameTestInstance` using `BuiltinTestFunctions.ALWAYS_PASS`, default environment, `minecraft:empty` structure, max/setup ticks of 1, required=false, and TestData defaults for rotation/manual/retry/sky-access/padding, cross-checked against the bundled vanilla `always_pass.json`; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 gametest_instances`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/gametest/framework/GameTestListener.java`. Rust `gametest_resources::listener_models` covers all four listener callbacks (`testStructureLoaded`, `testPassed`, `testFailed`, `testAddedForRerun`) with ordered payload recording for test ids, rerun copy ids, and runner identity; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 gametest_listener`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/gametest/framework/GameTestMainUtil.java`. Rust `gametest_resources::main_util_models` covers the default universe and level names, Java CLI options (`universe`, `report`, `tests`, `verify`, `repeatCount`, `packs`, `help`), unknown-option tolerance, help short-circuit, verify-without-tests exit behavior and exact message, verify/repeatCount ignore flag, server launch configuration values, optional tests/report/packs propagation, and datapack copy filtering for directories with `pack.mcmeta` plus `.zip` files. Actual Bootstrap/MinecraftServer spin, LevelStorageSource creation, filesystem reset/copy, and reporter installation are explicitly deferred behind `gametest-main-util-server-runtime`; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 gametest_main_util`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/gametest/framework/GameTestRunner.java`. Rust `gametest_resources::runner_models` covers runner builder defaults (`fromGameTestInfo` batcher, `IN_PLACE` existing spawner, `NOT_SET` new spawner, halt/clear flags false), flattened test-info collection, start/stop state, batch start/finish event ordering, environment activation/teardown, clear-between-batches forced-chunk cleanup marker, halt-on-error teardown plus ticker clear marker, stopped-runner rerun copying/rebatching, and `StructureSpawner` default behavior (`IN_PLACE` starts execution with delay 1, `NOT_SET` returns empty). Live ServerLevel operations, TestEnvironment activation object lifecycle, forced chunk mutation, GameTestTicker singleton mutation, ReportGameListener wiring, and structure placement/removal are explicitly deferred behind `gametest-runner-server-runtime`; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 gametest_runner`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/gametest/framework/GameTestSequence.java`. Rust `gametest_resources::sequence_models` covers event queue construction for wait-until, expected-delay, minimum-delay, idle/execute-after/execute-for, parent succeed/fail events, `tickAndContinue` swallowing assertion failures, `tickAndFailIfNotComplete` propagating failures, last-tick delay updates, exact sequence error keys for not-completed/minimum/invalid ticks, and `Condition` trigger state/error messages; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 gametest_sequence`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/gametest/framework/GameTestServer.java`. Rust `gametest_resources::server_models` pins the 433-line server source and covers constants/default policies (progress interval, position range, seed, max players, feature exclusions), test-selection filtering that excludes manual-only tests, empty-selection failure, direct/repeat/verify expansion including 4 rotations x 100 verify runs, repeat-count multiplication, server property overrides, start-position Y/range rules, failed-test log formatting with rotation names, and mock profile/name-id resolver behavior including offline-name fallback. Live MinecraftServer construction, WorldLoader/datapack loading, PackRepository mutation, LevelStorageSource/world creation, GameTestRunner/GridSpawner execution, tracker progress reporting, process exit, and crash handling are explicitly deferred behind `gametest-server-runtime`; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 gametest_server`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/gametest/framework/GameTestTicker.java`. Rust `gametest_resources::ticker_models` covers singleton identity, copy-on-write-style test list behavior, optional runner state, `IDLE`/`RUNNING`/`HALTING` transitions, add, idle clear stopping/nulling the runner and clearing tests, non-idle clear setting HALTING, duplicate runner warning text, tick no-op without runner, tick setting RUNNING, advancing tests, removing done tests, restoring IDLE, and recursive clear after HALTING; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 gametest_ticker`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/gametest/framework/GameTestTimeoutException.java`. Rust `gametest_resources::timeout_models` covers the exception extending `GameTestException`, stored message component, constructor runtime message from `message.getString()`, and `getDescription()` returning the stored component-equivalent message; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 gametest_timeout_exception`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/gametest/framework/GeneratedTest.java`. Rust `gametest_resources::generated_models` covers the record fields (`tests`, `functionKey`, `function`), `Registries.TEST_FUNCTION` resource-key creation from a function id, and the singleton `Map.of(id, testData)` constructor that also uses the test id as the function id; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 generated_test`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/gametest/framework/GlobalTestReporter.java`. Rust `gametest_resources::reporter_models` covers the static default `LogTestReporter` delegate, `replaceWith`, failure/success forwarding, and finish forwarding through the active delegate; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 reporter`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/gametest/framework/JUnitLikeTestReporter.java`. Rust `gametest_resources::reporter_models` covers timestamp initialization, testcase name/class/runtime fields, required failure versus optional skipped elements, failure message formatting with block position, success testcase creation, finish suite time conversion, and save destination tracking; actual DOM/XML emission remains a filesystem-backed reporter concern for the live GameTest runtime; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 reporter`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/gametest/framework/LogTestReporter.java`. Rust `gametest_resources::reporter_models` covers required error log formatting, optional warning log formatting, block-position/error interpolation, success no-op, and inherited default finish no-op; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 reporter`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/gametest/framework/MultipleTestTracker.java`. Rust `gametest_resources::multiple_tracker_models` covers test collection construction/add/remove, listener and failure-listener attachment to existing and newly tracked tests, failure listener notifications, failed required/optional counts and lists, done/total counts, done predicate, progress-bar character priority (` `, `_`, `+`, `X`, `x`), and `toString()` delegating to the progress bar; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 multiple_test_tracker`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/gametest/framework/ReportGameListener.java`. Rust `gametest_resources::reporter_models` covers attempt/success accounting, retry status formatting and rerun decisions, non-flaky pass/fail reporting, flaky success/rerun/exhaustion paths, block-entity success/error/mark-error updates, optional versus required chat formatting, global reporter success/failure calls, and listener attachment to rerun copies; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 report_game_listener`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/gametest/framework/RetryOptions.java`. Rust `gametest_resources::reporter_models::RetryOptionsModel` covers `NO_RETRIES`, `unlimitedTries`, `hasTriesLeft` with halt-on-failure semantics, and `hasRetries`; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 retry_options`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/gametest/framework/StructureGridSpawner.java`. Rust `gametest_resources::runner_models` covers grid spawner constants, constructor state, batch clearing/reset behavior, setting test block positions before preparation, empty optional return when preparation fails, start delay, row bounds growth, next-corner movement by structure X size plus spacing, max-X tracking, row wrapping by row Z size plus spacing, last-batch tracking, and optional return of spawned tests; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 structure_grid_spawner`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/gametest/framework/StructureUtils.java`. Rust `gametest_resources::runner_models` covers rotation step mapping and exact invalid-step error text, transformed far-corner and north-west-adjusted bounding-box geometry, empty test creation with clear-space side effects and CLEARED data, clear-space block/tick/event/entity effects, test block POI search, containing-test lookup, nearest-test lookup by Manhattan distance, looked-at test selection, and Java source sentinels for the live ServerLevel/POI/block-entity operations modeled in Rust; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 structure_utils`.
...
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/gametest/framework/TestData.java`. Rust `gametest_resources::generated_models::GameTestDataModel` covers the record fields, codec field names and defaults (`setup_ticks`, `required`, `rotation`, `manual_only`, retry counts, `sky_access`, `padding`), convenience constructors defaulting rotation/manual/retry/sky/padding values, and `map` preserving all fields while transforming the environment; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 test_data`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/gametest/framework/TestEnvironmentDefinition.java`. Rust `gametest_resources::environment_models` covers direct/holder codec registration shape, bootstrap ids and order, activation teardown, `AllOf` setup and reverse teardown, clock time save/set/restore, setup/teardown function execution and missing-function log text, game-rule save/restore, timeline environment replacement/restore, weather type ids/parameters/current-state detection/apply/restore, and codec-name dispatch for every environment variant; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 environment_`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/gametest/framework/TestFinder.java`. Rust `gametest_resources::runner_models` covers the combined test-instance/test-position finder, builder defaults for no functions/no structures, `createMultipleCopies` stream repetition, radius/nearest/allNearby/lookedAt position finders delegating to `StructureUtils`, failed-test filtering with the required-only flag, resource-selection test lookup, and source preservation; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 test_finder`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/gametest/framework/TestFunctionLoader.java`. Rust `gametest_resources::generated_models::TestFunctionLoaderRegistryModel` covers the static loader-list behavior, `registerLoader` insertion order, `runLoaders` iterating registered loaders, and each loader invoking the registry registration callback with function keys and functions; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 test_function_loader`.
...
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/nbt/ByteArrayTag.java`. Rust `Tag::ByteArray`, array size metadata, and SNBT/binary payload handling mirror Java ID/name/pretty-name, length-prefixed one-byte elements, `sizeInBytes = 24 + len`, copy/equality as value semantics, optional byte-array accessor behavior, and numeric `byteValue` coercion semantics for collection mutation; covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 primitive_array_tags`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/nbt/ByteTag.java`. Rust `Tag::Byte`, `NbtNumericValue`, and `tag_metadata` mirror Java byte tag ID/name/pretty-name, static payload size, `sizeInBytes`, cached zero/one/value factory behavior as value semantics, big-endian one-byte write/load payload, copy-as-self semantics, and numeric conversions; covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 scalar_nbt_tag_classes`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/nbt/CollectionTag.java`. Verified Java's shared collection contract for clear/set/add/remove/get/size, `isEmpty() == size() == 0`, iterator order, and no-such-element behavior; Rust primitive array tags expose equivalent value-level collection payload, size, copy, and ordered element behavior while `ListTag` remains tracked separately; covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 primitive_array_tags`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/nbt/CompoundTag.java`. Rust `compound_tag` plus the binary compound reader mirror Java's map-backed compound semantics: duplicate-key replacement on load/put, typed put helpers, optional and default typed getters, boolean-byte defaults, key/value/size/remove access, shallow/deep copy behavior, recursive compound merge, and Java `sizeInBytes` accounting through `Tag::size_in_bytes`; covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 compound_tag`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/nbt/DoubleTag.java`. Rust `Tag::Double`, `NbtNumericValue`, and `tag_metadata` mirror Java double tag ID/name/pretty-name, static payload size, `sizeInBytes`, zero factory behavior as value semantics, IEEE-754 big-endian write/load payload, copy-as-self semantics, and floor/wrap numeric conversions; covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 scalar_nbt_tag_classes`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/nbt/EndTag.java`. Rust `Tag::End` and `tag_metadata` mirror Java singleton end tag ID/name/pretty-name, zero-byte write/load payload, `sizeInBytes` 8, no-op skip behavior, and copy-as-self semantics; covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 scalar_nbt_tag_classes`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/nbt/FloatTag.java`. Rust `Tag::Float`, `NbtNumericValue`, and `tag_metadata` mirror Java float tag ID/name/pretty-name, static payload size, `sizeInBytes`, zero factory behavior as value semantics, IEEE-754 big-endian write/load payload, copy-as-self semantics, and floor/wrap numeric conversions; covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 scalar_nbt_tag_classes`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/nbt/IntArrayTag.java`. Rust `Tag::IntArray`, array size metadata, and SNBT/binary payload handling mirror Java ID/name/pretty-name, length-prefixed big-endian int elements, `sizeInBytes = 24 + 4 * len`, copy/equality as value semantics, optional int-array accessor behavior, and numeric `intValue` coercion semantics for collection mutation; covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 primitive_array_tags`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/nbt/IntTag.java`. Rust `Tag::Int`, `NbtNumericValue`, and `tag_metadata` mirror Java int tag ID/name/pretty-name, static payload size, `sizeInBytes`, small-value factory cache as value semantics, big-endian four-byte write/load payload, copy-as-self semantics, and numeric conversions; covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 scalar_nbt_tag_classes`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/nbt/ListTag.java`. Rust `Tag::List`, list write/read helpers, and list size metadata mirror Java ID/name/pretty-name, homogeneous element-type detection, mixed-list compound wrapper encoding, wrapper unwrapping on load, missing-type rejection for non-empty end lists, length-prefixed payloads, `sizeInBytes = 36 + 4 * len + child sizes`, copy/value semantics, and ordered SNBT output; covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 list_tag_matches`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/nbt/LongArrayTag.java`. Rust `Tag::LongArray`, array size metadata, and SNBT/binary payload handling mirror Java ID/name/pretty-name, length-prefixed big-endian long elements, `sizeInBytes = 24 + 8 * len`, copy/equality as value semantics, optional long-array accessor behavior, and numeric `longValue` coercion semantics for collection mutation; covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 primitive_array_tags`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/nbt/LongTag.java`. Rust `Tag::Long`, `NbtNumericValue`, and `tag_metadata` mirror Java long tag ID/name/pretty-name, static payload size, `sizeInBytes`, small-value factory cache as value semantics, big-endian eight-byte write/load payload, copy-as-self semantics, and numeric conversions; covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 scalar_nbt_tag_classes`.
...
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/nbt/NbtIo.java`. Rust `nbt_io` mirrors Java root compound I/O, gzip compressed read/write/parse helpers, nullable path read behavior, sync-style truncating path writes, `readAnyTag`/`writeAnyTag`, unnamed tag read/write, root compound validation, stream parse root `CONTINUE`/`BREAK`/`HALT` handling, payload visitor traversal, and string fallback for overlong modified-UTF strings; covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 nbt_io`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/nbt/NbtOps.java`. Rust `nbt_ops` mirrors Java's concrete NBT `DynamicOps` semantics: empty/list/map creation, scalar number/string/boolean creation and extraction, list collectors preserving primitive arrays until mixed, map merge/create/get/remove behavior with Java error surfaces and partial results, byte/int/long array streams, generic list creation, and record-builder-style compound prefix merge; generic cross-`DynamicOps` conversion, `MapLike`, and DFU `RecordBuilder` inheritance are explicitly TODO-deferred to the missing DynamicOps/DFU abstraction; covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 nbt_ops`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/nbt/NbtUtils.java`. Rust `nbt_utils` mirrors Java's NBT-local utility behavior: recursive/partial `compareNbt`, pretty-print layout and binary blob rendering, text-component rendering bridge, structure template pack/unpack, block-state string pack/unpack, Y-X-Z sorting for structure blocks/entities, and current/default data-version helpers; Java registry-backed block/fluid-state and DFU `Dynamic` overloads are explicitly TODO-deferred to the missing registry/DynamicOps subsystems; covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 nbt_utils`.
...
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/nbt/ShortTag.java`. Rust `Tag::Short`, `NbtNumericValue`, and `tag_metadata` mirror Java short tag ID/name/pretty-name, static payload size, `sizeInBytes`, small-value factory cache as value semantics, big-endian two-byte write/load payload, copy-as-self semantics, and numeric conversions; covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 scalar_nbt_tag_classes`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/nbt/SnbtGrammar.java`. Rust `snbt_grammar` now backs public `parse_snbt` and mirrors Java's NBT grammar behavior for control-character escaping, numeric lookahead, signed/unsigned integer suffixes, binary/decimal/hex numerals with underscore cleanup and leading-zero rejection, finite float/double parsing, quoted string escapes (`\s`, `\x`, `\u`, `\U`, common `\N{...}` aliases), true/false and built-in operation literals, duplicate-key-last compounds, trailing separators, typed primitive arrays with allowed element suffixes, and parser integration with `TagParser`/`NbtUtils`; full JDK Unicode character-name table and generic non-NBT `DynamicOps` parser construction are explicitly TODO-deferred to those missing abstractions; covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 snbt_grammar`, `snbt_parser`, `tag_parser`, and `nbt_utils`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/nbt/SnbtOperations.java`. Rust `snbt_operations` mirrors Java's `true`/`false` built-in IDs, `BuiltinKey` `id/argCount` display, built-in suggestions, `bool/1` conversion from boolean or numeric tags, `uuid/1` string parsing, Java `UUIDUtil.uuidToIntArray` most/least-significant-bit split, and error surfaces for invalid bool/UUID arguments; covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 snbt_operations`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/nbt/SnbtPrinterTagVisitor.java`. Rust `snbt_printer` mirrors Java's pretty SNBT visitor constants, scalar suffixes, string/key escaping, primitive-array spacing, root/template key ordering, configurable indentation, path-based no-indentation cases, and empty/end rendering behavior; covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 snbt_printer`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/nbt/StreamTagVisitor.java`. Rust `NbtStreamTagVisitor`, `StreamEntryResult`, and `StreamValueResult` mirror Java's streaming visitor method surface and `ENTER`/`SKIP`/`BREAK`/`HALT` plus `CONTINUE`/`BREAK`/`HALT` result enums; covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 stream_tag_visitor_interface`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/nbt/StringTag.java`. Rust `Tag::String`, modified-UTF helpers, `tag_metadata::string_size_in_bytes`, and `snbt_string` mirror Java empty-value factory semantics as value semantics, modified UTF read/write/skip payload shape, `sizeInBytes = 36 + 2 * UTF-16 length`, primitive/as-string behavior, copy-as-self semantics, Java `quoteAndEscape` quote selection, escape-without-quotes behavior, control-character escapes, and reserved boolean-key quoting in `StringTagVisitor`; covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 string_tag_matches`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/nbt/StringTagVisitor.java`. Rust `Tag::to_snbt` and `snbt_string` mirror Java visitor output for scalar suffixes, primitive arrays, strings, lists, `END`, reserved/regex-key quoting, and sorted compound entries via `Entry.comparingByKey`; covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 string_tag_visitor`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/nbt/Tag.java`. Rust `Tag`, `tag_metadata`, and `tag_access` mirror Java's shared tag interface constants, ID/type lookup, clone-backed copy contract, Java `sizeInBytes()` formulas, SNBT-backed string rendering, and default optional typed accessors for strings, numbers, booleans, primitive arrays, compounds, and lists; covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 tag_interface_matches`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/nbt/TagParser.java`. Rust `tag_parser` mirrors Java parser entry-point policy over the SNBT grammar: separator constants, full parse trailing-data rejection, argument parse cursor advancement without requiring end-of-input, compound-only full/argument helpers, non-compound rejection without consuming the argument cursor, and parse error surfaces; covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 tag_parser`.
...
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/nbt/TagVisitor.java`. Rust `NbtTagVisitor` and `Tag::accept_tag_visitor` mirror Java's one-method-per-concrete-tag visitor dispatch for string, numeric scalars, primitive arrays, list, compound, and end tags; covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 tag_visitor_dispatch`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/nbt/TextComponentTagVisitor.java`. Rust `text_component_tag_visitor` mirrors Java's component-producing NBT visitor: plain/rich styling roles, token text and syntax colors, scalar suffixes including uppercase long suffix, string/key quote splitting, configurable sorted/unsorted compound keys, inline vs wrapped list thresholds, array/depth folding, and empty/end rendering; covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 text_component_visitor`.
...
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/nbt/visitors/CollectFields.java`. Rust `CollectFieldsVisitor` mirrors Java's selected-field collector: compound-only root guard, wanted type prefilter including compounds, missing-field count, selected field removal, recursive `FieldTree` descent, `BREAK` once all fields are found, stack pop on matching container depth, and collected output containing only requested paths; covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 collect_fields_visitor`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/nbt/visitors/CollectToTag.java`. Rust `CollectToTagVisitor` mirrors Java's root/list/compound builder stack, scalar append callbacks, container entry handling, named compound key capture, depth reporting, `getResult`, container-end build/append behavior, and ListTag wrapper unwrapping; covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 collect_to_tag_visitor`.
...
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/nbt/visitors/SkipFields.java`. Rust `SkipFieldsVisitor` mirrors Java's `CollectToTag` subclass behavior: selector tree construction, type-sensitive selected-field `SKIP`, compound-path recursion through `FieldTree`, stack pop on matching container depth, and collected output preserving non-selected fields; covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 skip_fields_visitor`.
...
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/network/CipherBase.java`. Rust `network::encryption::CipherBase` mirrors Java's reusable heap input/output buffers around a stateful cipher update, returning decoded bytes for inbound data and appending encoded bytes to the outbound buffer; covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 cipher_base_decoder_and_encoder`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/network/CipherDecoder.java`. Rust `network::encryption::CipherDecoder` mirrors Java's `MessageToMessageDecoder<ByteBuf>` surface by decoding one input byte buffer through `CipherBase::decipher` and pushing one output buffer; covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 cipher_base_decoder_and_encoder`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/network/CipherEncoder.java`. Rust `network::encryption::CipherEncoder` mirrors Java's `MessageToByteEncoder<ByteBuf>` surface by forwarding one message through `CipherBase::encipher` into the outbound byte buffer while preserving cipher stream state; covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 cipher_base_decoder_and_encoder`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/network/ClientboundPacketListener.java`. Rust `network::dispatch::ClientboundPacketListener` mirrors Java's clientbound listener marker defaulting `flow()` to `PacketFlow.CLIENTBOUND`; covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 listener_surfaces`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/network/CompressionDecoder.java`. Rust `network::compression::CompressionDecoder` mirrors Java's VarInt uncompressed-length branch, zero-length pass-through, optional threshold/protocol-maximum validation, zlib inflate length check, and runtime threshold/validation update surface; covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 compression_decoder_and_encoder`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/network/CompressionEncoder.java`. Rust `network::compression::CompressionEncoder` mirrors Java's 8192-byte scratch-buffer model, protocol maximum guard, below-threshold zero data-length pass-through, zlib compression with declared uncompressed length, and get/set threshold surface; covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 compression_decoder_and_encoder`.
- [ ] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/network/Connection.java`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/network/ConnectionProtocol.java`. Rust `network::dispatch::ConnectionProtocol` mirrors Java's handshaking/play/status/login/configuration enum IDs and `id()` surface; covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 packet_flow_and_connection_protocol`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/network/DisconnectionDetails.java`. Rust `network::dispatch::DisconnectionDetails` mirrors Java's reason/report/bug-report-link record shape and reason-only constructor defaulting report and bug link to empty optionals; covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 java_packet_listener_defaults`.
- [ ] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/network/FriendlyByteBuf.java`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/network/HandlerNames.java`. Rust `network::handler_names::HandlerNames` mirrors Java's pipeline handler-name constants for compression, codec, config, framing, cipher, bundling, packet handler, timeout, legacy query, and latency handlers; covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 handler_names`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/network/HashedPatchMap.java`. Rust `network::play::HashedPatchMap` mirrors Java's component-patch hash creation, removed-component set equality, added-component count/key/hash matching, 256-entry stream-codec caps, and VarInt/i32 wire order; covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 hashed_patch_map_and_stack` and container-click packet codec tests.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/network/HashedStack.java`. Rust `network::play::HashedStack` mirrors Java's empty-stack sentinel matching, non-empty item/count/component-hash creation, item/count mismatch rejection, component patch matching, optional wire codec shape, and container-click integration; covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 hashed_patch_map_and_stack` and container-click packet codec tests.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/network/HiddenByteBuf.java`. Rust `network::local::HiddenByteBuf` mirrors Java's byte-buffer wrapper, pack/unpack behavior for byte buffers vs passthrough messages, and delegated reference-count/touch surface; covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 hidden_byte_buf`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/network/LocalFrameDecoder.java`. Rust `network::local::LocalFrameDecoder` mirrors Java's inbound local handler by unpacking `HiddenByteBuf` before forwarding the message; covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 local_frame_handlers`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/network/LocalFrameEncoder.java`. Rust `network::local::LocalFrameEncoder` mirrors Java's outbound local handler by packing byte buffers into `HiddenByteBuf` before writing; covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 local_frame_handlers`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/network/LpVec3.java`. Rust `network::lp_vec3` mirrors Java's low-precision Vec3 codec constants, NaN-to-zero and absolute clamp sanitization, tiny-vector zero marker, 15-bit component quantization, two-bit inline scale vs continuation VarInt scale suffix, little-endian low bytes plus big-endian high int layout, and decode unpacking behavior; covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 lp_vec3`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/network/MonitoredLocalFrameDecoder.java`. Rust `network::local::MonitoredLocalFrameDecoder` mirrors Java's monitored inbound local handler by unpacking `HiddenByteBuf`, accounting readable bytes on byte-buffer messages through `BandwidthDebugMonitor::on_receive`, then forwarding the unpacked message; covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 local_frame_handlers`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/network/PacketBundlePacker.java`. Rust `network::bundle::PacketBundlePacker` mirrors Java's current-bundler state, delimiter-start bundling, non-terminal verification inside bundles, packet accumulation, delimiter-finished bundle emission, passthrough outside bundles, terminal passthrough plus handler-removal signal, and terminal-in-bundle decoder error; covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 packet_bundle`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/network/PacketBundleUnpacker.java`. Rust `network::bundle::PacketBundleUnpacker` mirrors Java's bundler-info unbundle call, bundle expansion to delimiter/sub-packets/delimiter, non-bundle passthrough, and terminal handler-removal signal; covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 packet_bundle`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/network/PacketDecoder.java`. Rust `network::pipeline::PacketDecoderHandler` mirrors Java's readable-byte accounting, delegated protocol codec decode, `SkipPacketException` remaining-byte discard, leftover-byte IOException message shape, decoded packet output, and inbound terminal protocol-swap hook; covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 packet_decoder_handler`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/network/PacketEncoder.java`. Rust `network::pipeline::PacketEncoderHandler` mirrors Java's delegated protocol codec encode, written-byte accounting, skippable packet errors becoming `SkipPacketEncoderException`, fatal error propagation, and finally-run outbound terminal protocol-swap hook; covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 packet_encoder_handler`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/network/PacketListener.java`. Rust `network::dispatch::JavaPacketListener` mirrors Java's flow/protocol/disconnect/accepting-message surface, reported packet-error metadata, reason-only disconnection-info default, `shouldHandleMessage == isAcceptingMessages`, and connection crash-report protocol/flow details with no-op listener-specific extension; covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 java_packet_listener_defaults`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/network/PacketProcessor.java`. Rust `network::dispatch::PacketProcessor` mirrors Java's running-thread identity check, closed-state rejection message, FIFO queue scheduling, closed no-op processing, `shouldHandleMessage` gate, packet handle dispatch, OutOfMemory reported-exception propagation, and ordinary packet-error callback path; covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 packet_processor`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/network/PacketSendListener.java`. Rust `network::dispatch::PacketSendListener` mirrors Java's `thenRun` callback ordering, failure exception propagation, exceptional fallback packet send, and null-fallback exception path; covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 packet_send_listener`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/network/ProtocolInfo.java`. Rust `network::dispatch::ProtocolInfo` and `ProtocolInfoDetails` mirror Java's protocol id, packet flow, codec label, optional bundler info, details provider, and packet visitor `(PacketType, networkId)` listing surface; covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 protocol_info`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/network/ProtocolSwapHandler.java`. Rust `network::pipeline::ProtocolSwapHandler` mirrors Java's inbound terminal handling by disabling auto-read, inserting `inbound_config` before the current handler, and removing the current handler, plus outbound terminal handling by inserting `outbound_config` after the current handler and removing the current handler; covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 packet_decoder_handler` and `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 packet_encoder_handler`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/network/RateKickingConnection.java`. Rust `network::rate_limit::RateKickingConnection` mirrors Java's serverbound connection construction, stored packet-per-second limit, inherited `Connection.tickSecond` smoothed received-packet average, strict `averageReceivedPackets > limit` threshold, `disconnect.exceeded_packet_rate` translatable reason, clientbound disconnect send, `PacketSendListener.thenRun` disconnect callback, and read-only transition; covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 rate_kicking_connection`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/network/RegistryFriendlyByteBuf.java`. Rust `network::codec::RegistryFriendlyByteBuf` mirrors Java's `FriendlyByteBuf` wrapper construction from a source byte buffer plus registry access, retained registry-access accessor, and `decorator(registryAccess)` closure that wraps each source buffer with the same registry access; covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 registry_friendly_byte_buf`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/network/ServerboundPacketListener.java`. Rust `network::dispatch::ServerboundPacketListener` mirrors Java's serverbound listener marker defaulting `flow()` to `PacketFlow.SERVERBOUND`; covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 listener_surfaces`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/network/SkipPacketDecoderException.java`. Rust `network::dispatch::SkipPacketDecoderException` mirrors Java's decoder-exception skip wrapper with message and cause constructors plus the skip marker trait; covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 skip_packet_exceptions`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/network/SkipPacketEncoderException.java`. Rust `network::dispatch::SkipPacketEncoderException` mirrors Java's encoder-exception skip wrapper with message and cause constructors plus the skip marker trait; covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 skip_packet_exceptions`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/network/SkipPacketException.java`. Rust `network::dispatch::SkipPacketException` mirrors Java's empty marker interface shared by decoder and encoder skip exceptions; covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 skip_packet_exceptions`.
...
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/network/UnconfiguredPipelineHandler.java`. Rust `network::pipeline::UnconfiguredPipelineHandler` mirrors Java's inbound/outbound protocol setup tasks, handler replacement names, inbound auto-read restoration, non-packet passthrough, packet/byte-buffer rejection with release-before-error behavior, configuration-task release plus promise success, normal write passthrough, and inbound/outbound `andThen` task ordering; covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 unconfigured_pipeline_handler`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/network/Utf8String.java`. Rust `network::codec::read_string`/`write_string` mirror Java's VarInt encoded length, negative/too-large encoded length rejection, available-byte guard via exact read failure, UTF-8 decode, Java UTF-16 code-unit length limit, Netty `utf8MaxBytes(maxLength)` three-byte-per-code-unit encoded cap, byte-count VarInt write, and astral-character boundary behavior; covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 string_length_limit_counts_utf16_units_like_java`.
...
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/network/chat/ChatType.java`. Rust `chat_component::chat_type::ChatType` mirrors Java's seven resource keys, bootstrap chat/narration decorations, default chat decoration, bound name/target handling, and decorate/decorateNarration forwarding; registry sync NBT carries the same direct-message gray italic styles; covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 chat_type`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/network/chat/ChatTypeDecoration.java`. Rust `chat_component::chat_type::ChatTypeDecoration` mirrors Java's sender/direct-message/team factories, parameter id/name mapping with out-of-bounds id fallback to sender, sender/target/content selection, gray italic direct-message style, and translatable decoration output; covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 chat_type`.
...
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/network/chat/CommonComponents.java`. Rust `chat_component::common_components::CommonComponents` mirrors Java's empty, option, GUI button/link, disconnect/connect failure, newline, narration separator, ellipsis, and space components plus days/hours/minutes long arguments, boolean option status, local-server disconnect label choice, composed option status, generic option name/value, narration joining with `. ` separators, and line joining with newline separators; covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 common_components_match_java`.
...
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/network/chat/FontDescription.java`. Rust `chat_component::FontDescription` mirrors Java's `Resource`, `AtlasSprite`, and `PlayerSprite` variants, default `minecraft:default` resource, and resource-only `Identifier.CODEC` contract that rejects non-resource descriptions with the Java error prefix; covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 font_description_codec`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/network/chat/FormattedText.java`. Rust `chat_component::formatted_text::FormattedText` mirrors Java's empty, plain, styled, and composite formatted text values, `STOP_ITERATION`-style visitor short-circuiting, styled visits using `style.applyTo(parentStyle)`, and `getString` concatenation; covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 formatted_text`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/network/chat/HoverEvent.java`. Rust `chat_component::HoverEvent` plus `chat_component::hover_event` mirrors Java's three server-allowed actions, serialized action names, `ShowText` value field, `ShowItem` `ItemStackTemplate` fields (`id`, optional non-default `count`, optional `components` patch), `ShowEntity` entity tooltip info fields (`id`, `uuid`, optional `name`), and cached tooltip line order of name, translated entity type, and UUID; covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 hover_event`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/network/chat/LastSeenMessages.java`. Rust `chat_trust::LastSeenMessages`, `PackedLastSeenMessages`, and `LastSeenMessagesUpdateModel` mirror Java's empty/max-20 contract, signature-updater byte order, cache-based message-signature packing/unpacking, checksum calculation with nonzero fallback, packed unpack failure on unknown ids, fixed-bitset update checksum bypass, and cache body-push ordering; covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 last_seen_messages`.
...
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/network/protocol/BundleDelimiterPacket.java`. Rust `network::bundle::BundleDelimiterPacket` mirrors Java's pipeline-only delimiter packet whose `handle` path panics with the Java assertion message; covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 bundle_delimiter_packet`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/network/protocol/BundlePacket.java`. Rust `network::bundle::BundlePacket` mirrors Java's stored iterable sub-packet container and `subPackets` access surface; covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 packet_bundle_unpacker`.
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/network/protocol/BundlerInfo.java`. Rust `network::bundle::BundlePacker`/`PacketBundleUnpacker` mirror Java's 4096 packet bundle limit, delimiter-identity start/end behavior, constructor-on-closing-delimiter behavior, bundle unwrapping order, and non-bundle passthrough; covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 packet_bundle`.
...
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/network/protocol/PacketFlow.java`. Rust `network::dispatch::PacketFlow` mirrors Java's `SERVERBOUND("serverbound")`/`CLIENTBOUND("clientbound")` enum IDs, `getOpposite`, `id()`, and uppercase display form used in crash details; covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 packet_flow_and_connection_protocol`.
