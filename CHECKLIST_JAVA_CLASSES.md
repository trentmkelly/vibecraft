...
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/worldgen/BastionHousingUnitsPools.java`. Rust `data_worldgen_jigsaw_pools` covers housing unit center/path/stage/wall pool registration counts, housing and side-wall degradation processor sentinels, source line count, element counts, and Rust jigsaw bootstrap metadata; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 data_worldgen_jigsaw_pools`.
...
- [x] Audit, port or explicitly defer, and parity-test `decompiled-server-26.1.2/net/minecraft/data/worldgen/BastionSharedPools.java`. Rust `data_worldgen_jigsaw_pools` covers shared bastion gold/block/mob pool registration counts, bottom-rampart/generic processor sentinels, source line count, element counts, and Rust jigsaw bootstrap metadata; verified by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 data_worldgen_jigsaw_pools`.
