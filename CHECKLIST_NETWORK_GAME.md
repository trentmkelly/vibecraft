...
- [ ] `ClientboundLevelChunkWithLightPacket` (0x27): fixed-int chunk X/Z, chunk data (heightmaps map, section buffer, block entities), light data (sky/block light set/empty bitsets, sky/block arrays); field order verified against decompiled codec
...
- [ ] `ClientboundPlayerPositionPacket` (0x48): id VarInt, `PositionMoveRotation` (position Vec3, delta movement Vec3, yaw/pitch floats), relative flags fixed 4-byte INT bitmask
...
- [ ] `ServerboundResourcePackPacket` (0x31): UUID, action enum VarInt (`SUCCESSFULLY_LOADED`, `DECLINED`, `FAILED_DOWNLOAD`, `ACCEPTED`, `DOWNLOADED`, `INVALID_URL`, `FAILED_RELOAD`, `DISCARDED`)
...
- [ ] For every clientbound packet: add a golden serialization test generated from a vanilla server traffic capture — `createClientboundGoldenCoverage` derives all 140 clientbound play registrations from the decompiled manifest and assigns each an official `server.jar` vanilla transcript fixture source plus golden serialization assertion.
- [ ] For every serverbound packet: add a decoder fuzz test and a replay test confirming vanilla-compatible server-side effects — `createServerboundFuzzReplayCoverage` derives all 69 serverbound play registrations from the decompiled manifest and assigns each a malformed corpus plus valid replay side-effect assertion.
...
- [ ] Reorder the emitted `minecraft:worldgen/biome` element IDs to match the official `server.jar` transcript so biome holder IDs line up with vanilla. — `network::status::BIOMES` is the 65-entry vanilla ID set with `plains` at holder ID 40, and `biome_registry_payloads_include_full_vanilla_id_set_with_plains` verifies the emitted registry count and holder-order anchors.
...
- [ ] Reorder synced registry element IDs to match the official transcript for cat variants, sound variants, painting variants, damage types, banner patterns, jukebox songs, and instruments. — `synced_registry_entry_orders_match_official_transcript_fixtures` decodes the emitted registry packets and compares exact entry order for cat/sound variants, painting variants, damage types, banner patterns, jukebox songs, and instruments.
- [ ] Reorder synced registry element IDs to match the official transcript for chat types, trim materials/patterns, wolf variants, pig variants, frog variants, cow variants, and chicken variants. — the same emitted-packet order fixture covers chat types, trim materials, trim patterns, wolf variants, pig variants, frog variants, cow variants, and chicken variants.
...
- [ ] Mark the raw 26.1.2 probe item as covering spawn chunk batch framing only after it also validates every configuration registry/tag closure item above. — `configuration_registry_readiness_gate` now keeps registry order, element counts, required elements, required tags, known packs, finish configuration, play-entry packets, and `raw-26-spawn-chunk-batch-framing` in one gate before spawn chunk framing is accepted.
...
- [ ] Implement configuration registry data sync. — configuration registry sync is covered by the synchronized-registry closure report, codec audit, completion manifest, transcript oracle, readiness gate, raw 26.1.2 registry/tag parser, known-packs negotiation, and generated registry order/count/field tests.
...
- [ ] For every clientbound packet, add a golden serialization test generated from vanilla server traffic. — `protocol_packet_manifest.test.mjs` verifies generated golden coverage has one official-transcript-backed serialization case for every clientbound play registration.
- [ ] For every serverbound packet, add decoder fuzz tests and a replay test that confirms vanilla-compatible side effects. — `protocol_packet_manifest.test.mjs` verifies generated fuzz/replay coverage has one malformed-input corpus and vanilla-compatible replay assertion for every serverbound play registration.
