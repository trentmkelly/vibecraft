...
pub(super) fn parse_command_block_pos(
...
pub(super) fn parse_spawnable_block_pos(
    state: &ServerCommandState,
    x: &str,
    y: &str,
    z: &str,
) -> Result<BlockPos, CommandError> {
    let pos = parse_command_block_pos(state, x, y, z)?;
    if (-30_000_000..30_000_000).contains(&pos.x)
        && (-20_000_000..20_000_000).contains(&pos.y)
