...
fn chunk_generator_create_references_scans_nearby_valid_intersections() {
    assert_chunk_reference_key_and_writable_area_follow_vanilla();
    let target_chunk = ChunkPos { x: 2, z: -1 };
    let chunks = structure_reference_candidate_chunks();
    let references = super::super::structure_references_for_chunk(&chunks, target_chunk);
    assert_eq!(
        references,
        vec![super::super::StructureReferenceModel {
            structure: "minecraft:shipwreck",
            source_chunk_key: super::super::chunk_pos_key(target_chunk),
        }]
    );
    ...
    assert_eq!(
        ...
        &[super::super::chunk_pos_key(target_chunk)]
    );
    assert!(target_access.unsaved);
}

fn assert_chunk_reference_key_and_writable_area_follow_vanilla() {
    assert_eq!(
...
fn structure_reference_candidate_chunks() -> BTreeMap<i64, super::super::StructureAccessModel> {
    let mut source_access = super::super::StructureAccessModel::default();
    source_access.set_start_for_structure(
        "minecraft:shipwreck",
        structure_start_with_piece(
            "minecraft:shipwreck",
            ChunkPos { x: 2, z: -1 },
            super::super::StructureBoundingBoxModel {
                min_x: 31,
    ...
    out_of_range_access.set_start_for_structure(
        "minecraft:mineshaft",
        structure_start_with_piece(
            "minecraft:mineshaft",
            ChunkPos { x: 9, z: 0 },
    ...
    non_intersecting_access.set_start_for_structure(
...
fn structure_start_with_piece(
    ...
    super::super::StructureStartModel {
        ...
        chunk_pos,
        references: 0,
        pieces: vec![super::super::StructurePieceModel { bounding_box }],
    }
}
...
fn structure_access_serializes_chunk_structures_payload() {
    ...
    let Some((_, Tag::Compound(starts))) = root.iter().find(|(name, _)| name == "starts") else {
    ...
    let Some((_, Tag::Compound(references))) = root.iter().find(|(name, _)| name == "References")
    else {
...
fn buried_treasure_piece_generation_and_support_scan_match_vanilla() {
    ...
    assert!(super::super::buried_treasure_is_support(
        "minecraft:sandstone"
    ));
    assert!(super::super::buried_treasure_is_support(
        "minecraft:diorite"
