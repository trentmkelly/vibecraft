...
pub(super) fn assert_geode_nether_and_end_support() {
    assert_geode_config_and_thresholds();
    assert_geode_density_layers_and_placement();
    assert_iceberg_support();
    assert_blue_ice_support();
    assert_random_selector_support();
    assert_fill_layer_support();
    assert_end_island_and_replace_sphere_support();
    assert_basalt_pillar_support();
    assert_basalt_column_support();
    assert_delta_and_glowstone_support();
    assert_nether_vegetation_and_vine_support();
    super::feature_placement_support_end_structures_tests::assert_end_platform_and_gateway_support(
    );
    super::feature_placement_support_end_structures_tests::assert_chorus_and_end_podium_support();
    super::feature_placement_support_end_structures_tests::assert_end_spike_blocks_support();
    super::feature_placement_support_end_structures_tests::assert_end_crystal_support();
    assert_huge_fungus_support();
    assert_block_pile_support();
}

fn sample_geode_config() -> super::super::GeodeConfigurationModel {
    super::super::GeodeConfigurationModel {
...
fn sample_geode_thresholds(
    geode_config: &super::super::GeodeConfigurationModel,
) -> super::super::GeodeLayerThresholds {
    super::super::geode_layer_thresholds(
        geode_config.layers,
        geode_config.crack,
        4,
...
fn assert_geode_config_and_thresholds() {
    let geode_config = sample_geode_config();
    ...
    let geode_thresholds = sample_geode_thresholds(&geode_config);
    ...
    assert!(super::super::geode_should_generate_crack(
        geode_config.crack,
        0.94
    ));
}

fn assert_geode_density_layers_and_placement() {
    let geode_config = sample_geode_config();
    let geode_thresholds = sample_geode_thresholds(&geode_config);
...
fn assert_random_selector_support() {
...
fn assert_end_island_and_replace_sphere_support() {
    ...
    assert!(
        !end_island.contains(&super::super::EndIslandPlacementBlock {
    ...
    assert_eq!(
        super::super::validate_replace_sphere_config(
            super::super::ReplaceSphereConfigurationModel {
                target_state: "minecraft:netherrack",
                replace_state: "minecraft:basalt",
                radius_min: 8,
                radius_max: 7,
...
fn assert_basalt_pillar_support() {
...
fn assert_basalt_column_support() {
    ...
    assert_eq!(
        super::super::validate_column_feature_config(
            super::super::ColumnFeatureConfigurationModel {
                reach_min: 4,
                reach_max: 3,
                height_min: 5,
                height_max: 10,
    ...
    assert!(
        column_blocks.contains(&super::super::BasaltColumnPlacementBlock {
            pos: BlockPos { x: 1, y: 64, z: 0 },
        })
    );
    assert!(
...
fn assert_delta_and_glowstone_support() {
    ...
    assert_eq!(
        super::super::validate_delta_config(delta_config),
        Ok(delta_config)
    );
    assert_eq!(
...
fn assert_nether_vegetation_and_vine_support() {
...
fn assert_huge_fungus_support() {
    assert_huge_fungus_start_and_height();
    assert_huge_fungus_stem_support();
    assert_huge_fungus_hat_support();
}

fn assert_huge_fungus_start_and_height() {
...
fn assert_huge_fungus_stem_support() {
...
fn assert_huge_fungus_hat_support() {
