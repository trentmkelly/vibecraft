...
fn height_provider_sampling_envelopes_follow_vanilla_edge_cases() {
    ...
    assert_vertical_anchor_resolution_matches_java(context);
    assert_constant_and_uniform_height_provider_sampling(context);
    assert_biased_height_provider_sampling(context);
    assert_weighted_height_provider_sampling(context);
}

fn assert_vertical_anchor_resolution_matches_java(context: WorldGenerationHeightContext) {
...
fn assert_constant_and_uniform_height_provider_sampling(context: WorldGenerationHeightContext) {
...
fn assert_biased_height_provider_sampling(context: WorldGenerationHeightContext) {
...
fn assert_weighted_height_provider_sampling(context: WorldGenerationHeightContext) {
...
fn block_predicate_core_evaluators_follow_vanilla_boolean_rules() {
    ...
    assert_basic_block_predicate_evaluators(grass);
    assert_composite_block_predicate_evaluators(grass);
    ...
    assert_offset_block_predicate_evaluators(grass, water_below);
    assert_composite_offset_block_predicate_evaluators(grass, water_below);
}

fn assert_basic_block_predicate_evaluators(grass: BlockPredicateContext) {
...
fn assert_composite_block_predicate_evaluators(grass: BlockPredicateContext) {
...
fn assert_offset_block_predicate_evaluators(
    grass: BlockPredicateContext,
    water_below: BlockPredicateContext,
) {
    assert_offset_state_block_predicate_evaluators(grass, water_below);
    assert_offset_survival_block_predicate_evaluators(grass, water_below);
    assert_offset_sturdy_face_block_predicate_evaluators(grass, water_below);
    assert_offset_out_of_bounds_block_predicate_evaluators(grass, water_below);
}

fn assert_offset_state_block_predicate_evaluators(
    grass: BlockPredicateContext,
    water_below: BlockPredicateContext,
) {
...
fn assert_offset_survival_block_predicate_evaluators(
    grass: BlockPredicateContext,
    water_below: BlockPredicateContext,
) {
    assert!(super::super::block_predicate_test_with_vertical_context(
...
fn assert_offset_sturdy_face_block_predicate_evaluators(
    grass: BlockPredicateContext,
    water_below: BlockPredicateContext,
) {
    assert!(super::super::block_predicate_test_with_vertical_context(
...
fn assert_offset_out_of_bounds_block_predicate_evaluators(
    grass: BlockPredicateContext,
    water_below: BlockPredicateContext,
) {
    assert!(!super::super::block_predicate_test_with_vertical_context(
...
fn assert_composite_offset_block_predicate_evaluators(
    grass: BlockPredicateContext,
    water_below: BlockPredicateContext,
) {
...
fn placement_modifier_registry_and_core_positions_follow_vanilla_rules() {
    assert_placement_modifier_registry_matches_vanilla();
    let origin = placement_modifier_origin();
    assert_count_and_randomized_placement_modifiers_follow_vanilla(origin);
    assert_fixed_placement_modifiers_follow_vanilla(origin);
    let placement_context = placement_context_model();
    assert_contextual_placement_modifiers_follow_vanilla(origin, placement_context);
    assert_placed_feature_pipeline_follows_vanilla(origin, placement_context);
}

fn assert_placement_modifier_registry_matches_vanilla() {
...
fn placement_modifier_origin() -> BlockPos {
    BlockPos {
        x: 32,
        y: 70,
        z: -16,
...
fn placement_context_model() -> PlacementContextModel {
...
fn assert_count_and_randomized_placement_modifiers_follow_vanilla(origin: BlockPos) {
    assert_eq!(
...
fn assert_fixed_placement_modifiers_follow_vanilla(origin: BlockPos) {
...
fn assert_contextual_placement_modifiers_follow_vanilla(
    origin: BlockPos,
    placement_context: PlacementContextModel,
) {
    assert_height_placement_modifiers_follow_vanilla(origin, placement_context);
    assert_environment_scan_placement_modifier_follows_vanilla(placement_context);
    assert_filter_placement_modifiers_follow_vanilla(origin, placement_context);
}

fn assert_height_placement_modifiers_follow_vanilla(
    origin: BlockPos,
    placement_context: PlacementContextModel,
) {
    assert_eq!(
...
fn assert_environment_scan_placement_modifier_follows_vanilla(
    placement_context: PlacementContextModel,
) {
...
fn assert_filter_placement_modifiers_follow_vanilla(
    origin: BlockPos,
    placement_context: PlacementContextModel,
) {
...
fn assert_placed_feature_pipeline_follows_vanilla(
    origin: BlockPos,
    placement_context: PlacementContextModel,
) {
    static TREE_PIPELINE_MODIFIERS: &[PlacementModifier] = &[
    ...
    let pipeline_origin = BlockPos {
        x: 32,
        y: 0,
        z: -16,
    };
    let pipeline_rolls = &[(0, 0, 0), (0, 0, 0), (3, 4, 0), (0, 0, 0)];

    assert_eq!(
        super::super::placed_feature_positions(
            TREE_PIPELINE_MODIFIERS,
            pipeline_origin,
            placement_context,
            pipeline_rolls,
        ),
    ...
    match super::super::placed_feature_invocations(
        "minecraft:oak",
        TREE_PIPELINE_MODIFIERS,
        pipeline_origin,
        placement_context,
        ...
        Err(error) => panic!("expected oak placement invocations, got error: {error}"),
    }
    match super::super::placed_feature_invocations(
        ...
        Ok(_) => panic!("expected unknown configured feature error"),
        Err(error) => assert_eq!(error, "unknown configured feature minecraft:not_a_feature"),
    }
}
