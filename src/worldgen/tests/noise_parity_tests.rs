...
fn random_state_normal_noise_instantiation_plan_matches_java_wiring() {
    ...
    assert!(super::super::random_state_normal_noise_instantiation_plan(
        12345, overworld, "missing"
    )
    .is_none());
}
...
fn perlin_noise_construction_plan_matches_vanilla_octave_wiring() {
    ...
    let legacy_plan =
        super::super::perlin_noise_construction_plan(nether_temperature, false).unwrap();
...
fn perlin_noise_snapshot_samples_match_vanilla_multi_octave_accumulation() {
    ...
    let temperature_random =
        super::super::random_state_normal_noise_instantiation_plan(12345, overworld, "temperature")
            ...
            .random;
    let temperature =
        super::super::perlin_noise_snapshot(temperature_random, temperature_parameters, true)
            .unwrap();
    ...
    assert!(
        (super::super::perlin_noise_edge_value(&temperature, 2.0) - 1.7777777777777777).abs()
            < 1e-12
    );
    ...
    assert!(
        (super::super::perlin_noise_max_broken_value(&legacy_nether, 0.25) - 2.25).abs() < 1e-12
    );
...
fn normal_noise_snapshot_samples_match_vanilla_dual_perlin_composition() {
    ...
    let temperature_random =
        super::super::random_state_normal_noise_instantiation_plan(12345, overworld, "temperature")
            ...
            .random;
    let temperature =
        super::super::normal_noise_snapshot(temperature_random, temperature_parameters, true)
            .unwrap();
...
fn perlin_and_normal_noise_derivatives_propagate_scaled_octave_sums() {
    ...
    assert!(
        (sample - super::super::perlin_noise_sample(&legacy_nether, 1.25, -3.5, 8.75, 0.0, 0.0))
            .abs()
            < 1e-12
    );
