...
pub(super) fn assert_tree_size_and_basic_tree_support() {
    ...
    assert!(super::super::tree_trunk_free_pos(
    ...
    assert!(super::super::tree_can_place(
        super::super::TreeCanPlaceInput {
            ...
            build_min_y: -64,
            build_max_y: 320,
            rows: &[&free_row, &free_row, &free_row, &free_row, &free_row, &free_row, &free_row,],
            ignore_vines: true,
        }
    ));
    assert!(!super::super::tree_can_place(
    ...
    assert!(!tree_plan.blocks.iter().any(|block| {
        block.kind == TreePlacementBlockKind::Leaves && block.pos == BlockPos { x: 7, y: 71, z: 7 }
    }));
    assert!(tree_plan.blocks.iter().any(|block| {
        block.kind == TreePlacementBlockKind::Leaves && block.pos == BlockPos { x: 6, y: 68, z: 8 }
    }));
    assert!(tree_plan.blocks.iter().any(|block| {
