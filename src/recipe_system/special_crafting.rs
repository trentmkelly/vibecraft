...
pub struct MapCraftingData {
    pub scale: u8,
    pub exploration_map: bool,
}

/// A read-only, map-id-keyed view of the level's map saved-data, supplying the
/// crafting menu with the map metadata recipes need (Java reads this via `level`).
///
...
impl MapDataStore {
    /// Register a map's saved-data (the server populates this from the map store).
    #[allow(dead_code)]
    pub fn insert(&mut self, map_id: i32, data: MapCraftingData) {
        self.maps.insert(map_id, data);
    }

    pub fn get(&self, map_id: i32) -> Option<MapCraftingData> {
...
impl RecipeMap {
    ...
    /// `MapExtendingRecipe` result, driven by the loaded special recipe + the level's
    /// map saved-data (`maps`). Returns the id + outcome of the registered
    /// map-extending recipe when the grid is the 8-paper-around-a-filled-map pattern
    /// and the centre map is extendable. Consulted by the crafting menu after the
    /// id-only special path, since this needs the map metadata that path can't see.
    pub fn map_extending_result(
        &self,
        grid: &[ItemStack],
        maps: &MapDataStore,
    ) -> Option<(&'static str, SpecialCraftOutcome)> {
        self.values().iter().find_map(|holder| {
            matches!(
                &holder.recipe,
                RecipeKind::Special {
            ...
            .then(|| map_extending_result(grid, maps).map(|outcome| (holder.id, outcome)))
            .flatten()
        })
    }
}
...
/// `MapExtendingRecipe` — a filled map in the centre of a 3×3 grid surrounded by 8
/// paper, where the map's saved data is a non-exploration map below the max scale,
/// yields a copy of the map flagged `MAP_POST_PROCESSING = SCALE` (zoomed out one
/// level when the player takes it). 1:1 with `MapExtendingRecipe.matches`/`assemble`.
fn map_extending_result(grid: &[ItemStack], maps: &MapDataStore) -> Option<SpecialCraftOutcome> {
    if grid.len() != 9 {
        return None;
    }
    // The fixed pattern: centre (index 4) is the filled map, the other 8 are paper.
    for (index, stack) in grid.iter().enumerate() {
        ...
        if stack.item_id() != expected {
            return None;
        }
    }
    let map = &grid[4];
    let map_id = match map.component("minecraft:map_id") {
        Some(ItemComponent::MapId(id)) => *id,
        // No saved-data id → `MapItem.getSavedData` is null → no match.
        _ => return None,
    };
    let data = maps.get(map_id)?;
    if data.exploration_map || data.scale >= 4 {
        return None;
    }

    // `createWithOriginalComponents(filled_map, sourceMap)` + MAP_POST_PROCESSING=SCALE.
    let mut result = map.transmute_copy("minecraft:filled_map", 1);
    result.set_component(ItemComponent::MapPostProcessing(MapPostProcessing::Scale));
