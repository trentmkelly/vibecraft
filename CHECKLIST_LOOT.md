...
- [x] Implement chest/container loot: one-time realization from loot table; `RandomizableContainerBlockEntity.unpackLootTable()` on first open; seed stored per block entity - Java `RandomizableContainer.unpackLootTable` loads the saved loot table/seed, clears the table before filling, builds CHEST loot params with ORIGIN plus optional player luck/THIS_ENTITY, and `LootTable.fill` shuffles empty slots, splits/shuffles generated stacks, and warns on overfill; Rust `RandomizableContainerLoot` stores `loot_table`/`loot_table_seed`, `unpack_once` clears future realization via `unpacked`, evaluates `LootSurface::ChestOpen` with the stored seed and origin, and `LootTable::fill_container` shuffles slots/items and records the same overfill warning; covered by `randomizable_container_loot_realizes_table_once_and_preserves_seed`, `container_fill_shuffles_once_and_reports_overfill`, and `cargo test -q loot_system`.
