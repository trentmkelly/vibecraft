...
- [ ] Implement score providers: `ContextScoreboardNameProvider`, `FixedScoreboardNameProvider` - `loot_system::ScoreProvider` resolves context-backed scoreboard names and fixed names; covered by `score_and_nbt_providers_resolve_context_and_storage_values` and `cargo test -q loot_system`.
