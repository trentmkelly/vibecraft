...
- [ ] Implement skylight effect of weather: CLEAR = 15, RAIN = 10, THUNDER = 10 — `weather::effective_sky_light` maps clear/rain/thunder to 15/10/10 as the legacy approximation. **Verification (held open):** 26.1.2 no longer uses a flat −5. `Level.tickTime` sets `skyDarken = (int)(15 - EnvironmentAttributes.SKY_LIGHT_LEVEL)`, and `WeatherAttributes` modifies `SKY_LIGHT_LEVEL` via `FloatModifier.ALPHA_BLEND` toward 4.0 (rain alpha 0.3125, thunder alpha 0.52734375) scaled by rain/thunder level — which does not reduce to a clean 10. A faithful port requires the EnvironmentAttributes/FloatModifier subsystem; `TODO` left in `weather::sky_darken_amount`.
