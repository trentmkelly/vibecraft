...
- [x] Add a Mineflayer offline-mode spawnpoint-persistence test that sets bed and anchor spawn points, reconnects, dies, respawns, and verifies saved spawn state and missing-spawn fallback match vanilla. — covered by `harness/mineflayer/spawnpoint_persistence.mjs` and `node --test spawnpoint_persistence.test.mjs`.
