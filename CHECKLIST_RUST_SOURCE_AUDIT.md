...
- [x] Audit Rust implementation file `src/criterion_effects_changed.rs` against the Java source and checklist coverage. This file is the complete Rust parity model for `EffectsChangedTrigger.java` and `MobEffectsPredicate.java`; optional effects/source predicates, effects-before-source matching, null-source rejection, source validation labels, `hasEffects`/`gotEffectsFrom` factories, entity/living/raw-map mob-effect predicate overloads, required-effect map matching, builder shape, and scoped mob-effect instance checks are covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 effects_changed` and `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 mob_effects`.
...
- [x] Audit Rust implementation file `src/criterion_item_durability.rs` against the Java source and checklist coverage. This file is the complete Rust parity model for `ItemDurabilityTrigger.java`; item predicate ordering, `maxDamage - newDurability` remaining durability, `damageValue - newDurability` delta, default `ANY` bounds, and both `changedDurability` factory shapes are covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 item_durability`.
- [x] Audit Rust implementation file `src/criterion_item_predicate.rs` against the Java source and checklist coverage. This file is the complete Rust parity model for `ItemPredicate.java`; optional item holder sets, count bounds, component matcher delegation, builder defaults, direct item holder construction, tag lookup/get-or-throw behavior, and builder field preservation are covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 item_predicate`.
- [x] Audit Rust implementation file `src/criterion_killed_by_arrow.rs` against the Java source and checklist coverage. This file is the complete Rust parity model for `KilledByArrowTrigger.java`; fired-weapon predicate gating, unique entity type counting, distinct victim-context consumption, victim matching failure behavior, unique bound ordering, crossbow factory overloads, and victim validation paths are covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 killed_by_arrow`.
- [x] Audit Rust implementation file `src/criterion_killed_trigger.rs` against the Java source and checklist coverage. This file is the complete Rust parity model for `KilledTrigger.java`; killing-blow-before-entity matching, server-player damage-source context, omitted defaults, entity wrapping and validation, all player/entity killed factory overloads, sculk catalyst trigger id, and damage-source builder build-before-store behavior are covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 killed_trigger`.
- [x] Audit Rust implementation file `src/criterion_levitation.rs` against the Java source and checklist coverage. This file is the complete Rust parity model for `LevitationTrigger.java`; start-to-player distance matching before duration bounds, omitted distance/default duration behavior, delegated distance predicate math, duration bounds, and `levitated` factory fields are covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 levitation`.
- [x] Audit Rust implementation file `src/criterion_light_predicate.rs` against the Java source and checklist coverage. This file is the complete Rust parity model for `LightPredicate.java`; loaded-position gating, max local raw brightness lookup, int-bound matching, default `ANY` light bound, and builder field preservation are covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 light_predicate`.
- [x] Audit Rust implementation file `src/criterion_lightning_bolt_predicate.rs` against the Java source and checklist coverage. This file is the complete Rust parity model for `LightningBoltPredicate.java`; lightning-only entity matching, blocks-set-on-fire bounds, optional struck-entity predicates with level/position context, `blockSetOnFire` shape, and registered lightning sub-predicate codec identity are covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 lightning_bolt_predicate`.
- [x] Audit Rust implementation file `src/criterion_lightning_strike.rs` against the Java source and checklist coverage. This file is the complete Rust parity model for `LightningStrikeTrigger.java`; lightning and nearby entity loot-context matching, omitted defaults, lightning-before-bystander ordering, bystander any-match behavior, factory wrapping/trigger id, and validation labels are covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 lightning_strike`.
- [x] Audit Rust implementation file `src/criterion_location_predicate.rs` against the Java source and checklist coverage. This file is the complete Rust parity model for `LocationPredicate.java`; optional record fields, position bounds/default omission, dimension checks, floor-to-block-position conversion, loaded guards for biome/structure/smokey, delegated light/block/fluid checks, sky visibility, and builder helpers are covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 location_predicate`.
- [x] Audit Rust implementation file `src/criterion_loot_table_trigger.rs` against the Java source and checklist coverage. This file is the complete Rust parity model for `LootTableTrigger.java`; trigger forwarding, codec field surface, `lootTableUsed` factory trigger id/player default, and exact resource-key matching are covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 loot_table_trigger`.
- [x] Audit Rust implementation file `src/criterion_min_max_bounds.rs` against the Java source and checklist coverage. This file is the complete Rust parity model for `MinMaxBounds.java`; generic bounds factories/accessors, range/point/codec/stream shapes, command-reader parsing/errors, contained-range validation, double/int squared matching, and float-degree reader behavior are covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 min_max_bounds`.
- [x] Audit Rust implementation file `src/criterion_movement_predicate.rs` against the Java source and checklist coverage. This file is the complete Rust parity model for `MovementPredicate.java`; codec fields/defaults, signed component checks, total/horizontal squared speed checks, absolute vertical speed checks, fall-distance matching, and static factory helpers are covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 movement_predicate`.
- [x] Audit Rust implementation file `src/criterion_nbt_predicate.rs` against the Java source and checklist coverage. This file is the complete Rust parity model for `NbtPredicate.java`; SNBT/stream compound preservation, nullable tag rejection, Java partial compound/list comparison, custom-data component matching, entity save-without-id matching, and player selected-item injection are covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 nbt_predicate`.
- [x] Audit Rust implementation file `src/criterion_picked_up_item_trigger.rs` against the Java source and checklist coverage. This file is the complete Rust parity model for `PickedUpItemTrigger.java`; codec fields, nullable picked-up-by entity loot context creation, item-before-entity matching, omitted defaults, both thrown-item pickup factory trigger ids/player shapes, and entity validation labels are covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 picked_up_item`.
- [x] Audit Rust implementation file `src/criterion_player_hurt_entity.rs` against the Java source and checklist coverage. This file is the complete Rust parity model for `PlayerHurtEntityTrigger.java`; codec fields, victim loot-context creation, damage-before-entity matching, omitted defaults, all factory overload trigger ids/field shapes, optional entity wrapping, and entity validation labels are covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 player_hurt_entity`.
- [x] Audit Rust implementation file `src/criterion_player_interact.rs` against the Java source and checklist coverage. This file is the complete Rust parity model for `PlayerInteractTrigger.java`; codec fields, interacted-entity loot-context creation, item-before-entity matching, omitted defaults, item-used and equipment-sheared factory overload trigger ids/player shapes, item builder storage, and entity validation labels are covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 player_interact`.
- [x] Audit Rust implementation file `src/criterion_player_predicate.rs` against the Java source and checklist coverage. This file is the complete Rust parity model for `PlayerPredicate.java`; player-only matching, codec/sub-predicate identity, level/food/gamemode/stat/recipe/advancement/input checks, looking-at ray-result gates, advancement predicate variants, and builder field preservation are covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 player_predicate`.
- [x] Audit Rust implementation file `src/criterion_player_trigger.rs` against the Java source and checklist coverage. This file is the complete Rust parity model for `PlayerTrigger.java`; codec field surface, unconditional `trigger(player)` instance predicate, location overload wrapping, empty-player trigger factories, and `walkOnBlockWithEquipment` feet-equipment plus stepping-on-block composition are covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 player_trigger`.
- [x] Audit Rust implementation file `src/criterion_raider_predicate.rs` against the Java source and checklist coverage. This file is the complete Rust parity model for `RaiderPredicate.java`; codec defaults, raider sub-predicate type, `CAPTAIN_WITHOUT_RAID`, exact raider flag matching, non-raider rejection, and ignored level/position arguments are covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 raider_predicate`.
- [x] Audit Rust implementation file `src/criterion_recipe_crafted.rs` against the Java source and checklist coverage. This file is the complete Rust parity model for `RecipeCraftedTrigger.java`; codec fields/default ingredients, factory trigger ids, item predicate builder storage, recipe-id gate, mutable remaining ingredient matching, one-stack consumption, and extra-stack allowance are covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 recipe_crafted`.
- [x] Audit Rust implementation file `src/criterion_recipe_unlocked.rs` against the Java source and checklist coverage. This file is the complete Rust parity model for `RecipeUnlockedTrigger.java`; codec fields, static factory trigger id and empty player predicate, and exact `RecipeHolder.id()` key matching are covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 recipe_unlocked`.
- [x] Audit Rust implementation file `src/criterion_sheep_predicate.rs` against the Java source and checklist coverage. This file is the complete Rust parity model for `SheepPredicate.java`; optional sheared codec field, sheep sub-predicate type, non-sheep rejection, optional/equality sheared matching, and `hasWool()` are covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 sheep_predicate`.
- [x] Audit Rust implementation file `src/criterion_shot_crossbow.rs` against the Java source and checklist coverage. This file is the complete Rust parity model for `ShotCrossbowTrigger.java`; codec fields, trigger id, both static factory shapes, item predicate builder storage, omitted-item match-any behavior, and item predicate matching delegation are covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 shot_crossbow`.
- [x] Audit Rust implementation file `src/criterion_simple_trigger.rs` against the Java source and checklist coverage. This file is the complete Rust parity model for `SimpleCriterionTrigger.java`; listener identity-map behavior, add/remove cleanup, trigger filtering and player predicate checks, copy-before-run awarding, and `SimpleInstance` player validation are covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 simple_trigger`.
- [x] Audit Rust implementation file `src/criterion_single_component_item_predicate.rs` against the Java source and checklist coverage. This file is the complete Rust parity model for `SingleComponentItemPredicate.java`; component type exposure and default component-getter matching/delegation behavior are covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 single_component_item_predicate`.
- [x] Audit Rust implementation file `src/criterion_slide_down_block.rs` against the Java source and checklist coverage. This file is the complete Rust parity model for `SlideDownBlockTrigger.java`; codec fields/validation, factory trigger id, omitted-default matching, block-before-state filtering, and state property matching are covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 slide_down_block`.
- [x] Audit Rust implementation file `src/criterion_slime_predicate.rs` against the Java source and checklist coverage. This file is the complete Rust parity model for `SlimePredicate.java`; size codec default, sized factory, slime sub-predicate type, non-slime rejection, and `getSize()` bounds matching are covered by `RUSTCRAFT_SKIP_LINE_CHECK=1 cargo test -q -j 1 slime_predicate`.
