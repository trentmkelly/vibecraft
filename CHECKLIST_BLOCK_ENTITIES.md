...
- [ ] Implement `BlockEntity.getUpdateTag()` returning only the NBT subset sent in `BlockEntityData` network packets (not the full save NBT): `BlockEntity::get_update_tag()` returns empty update tags for container block entities and save-without-metadata tags for visible update entities; covered by `update_packets_use_position_type_and_update_tag`
