...
- [ ] Add unit test: save NBT → load NBT round-trip preserves all fields for every block entity type: `save_load_round_trip_preserves_generic_fields_for_every_type` iterates all registered block entity types, serializes full metadata with custom data/components, reloads through `load_static()`, and verifies type, block state, payload maps, and transient flags
