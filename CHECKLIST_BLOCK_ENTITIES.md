...
- [ ] Implement `BlockEntity.getUpdatePacket()` returning a `ClientboundBlockEntityDataPacket` at the correct packet ID: `BlockEntity::get_update_packet()` carries position, type, and update tag, and `network::play::ClientboundBlockEntityDataPacket` serializes the play packet shape
