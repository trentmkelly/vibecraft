...
- [ ] Implement `BlockEntity.handleUpdateTag(tag)` applying the network-received subset on the simulated client side: `BlockEntity::handle_update_tag()` replaces custom data/components from the network tag while ignoring metadata fields (`id`, `x`, `y`, `z`); covered by `handle_update_tag_applies_network_subset_without_metadata`
- [ ] Implement `BlockEntityTicker` dispatch: server-tick and client-tick registrations independently null-checked per type: `BlockEntity::tick(client_side)` checks `BlockEntityTypeInfo.tick_kind`, level attachment, and removal state; covered by `ticking_requires_level_side_match_and_not_removed`
